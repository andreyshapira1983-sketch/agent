...
def test_auto_runtime_corporate_learning_uses_doctrine_docs_even_when_rotation_hits_core(
    workspace: Path,
    monkeypatch,
):
    ...
    learning = report.tasks[1]
    assert learning.task.kind == "learn"
...
def test_auto_runtime_confidence_learning_uses_verifier_sources_even_when_rotation_hits_scripts(
    workspace: Path,
    monkeypatch,
):
    ...
    report = AutonomousRuntime(agent, workspace=workspace).run(
        AutonomousRuntimeConfig(
            goal="debug low-confidence gate evidence citations verified unverified",
            dry_run=True,
            limit=2,
            include_tests=False,
            learning_limit=4,
...
def test_dry_run_goal_uses_gateway_simulate_and_restores(workspace: Path):
    ...
    assert seen["gateway_dry_run"] is True
    assert seen["gateway_path"] == "runtime"
    assert "file_write" not in seen["blocked"]
    assert "shell_exec" not in seen["blocked"]
    assert agent.gateway_dry_run is False
    assert agent.gateway_path == "repl"
    assert agent.policy.blocked_tools == frozenset()

    # Runtime logs gateway context on the agent trace (not legacy tools_blocked).
    log_path = agent.log.path
    events = [
        json.loads(line)
        for line in log_path.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    gw_events = [e for e in events if e.get("event") == "autonomous_goal_gateway"]
    assert gw_events
    payload = gw_events[-1].get("payload", {})
    assert payload.get("gateway_path") == "runtime"
    assert payload.get("gateway_dry_run") is True

    seen.clear()
    runtime._task_goal(task, AutonomousRuntimeConfig(dry_run=False))
    assert seen["gateway_dry_run"] is False
    assert "file_write" not in seen["blocked"]
    assert agent.policy.blocked_tools == frozenset()
...
def test_dry_run_and_no_tests_block_combines_and_restores(workspace: Path):
    """dry_run (gateway simulate) and include_tests=False (policy block) together
    apply during goal execution, and state is restored afterwards."""
    from core.autonomous_runtime import AutonomousTask
...
def test_goal_path_blocks_subagent_and_network(workspace: Path):
    ...
    agent.run = _capture  # type: ignore[method-assign]
    runtime = AutonomousRuntime(agent, workspace=workspace)
    task = AutonomousTask(kind="goal", description="find the next safe step")
    ...
    for dry in (True, False):
        ...
        assert _AUTONOMOUS_GOAL_BLOCKED_TOOLS <= seen["blocked"]
        # Planner surface is pruned too (not just policy-blocked at execution).
        assert _AUTONOMOUS_GOAL_BLOCKED_TOOLS <= seen["hidden"]
        # Effect tools are NOT policy-blocked (gateway governs those).
        assert "file_write" not in seen["blocked"]
