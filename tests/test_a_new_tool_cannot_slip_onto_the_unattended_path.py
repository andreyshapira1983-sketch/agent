...
_DELIBERATELY_OPEN: dict[str, str] = {
    ...
    "memory_recall": "чтение собственной долговременной памяти по слову: не больше 5 записей, каждая помечена LOW-TRUST с источником; открыто 2026-09-04 словом оператора («А так тебя устраивает? Да») — без чтения экзамен STORE→RETRIEVE→USE несдаваем",
    "model_route": "его дверь к собственной политике маршрутов (слово оператора 2026-09-04 22:20): роль → провайдер из разрешённого пула + модель + причина без секретов; ключ подставляет доверенный роутер; маршруты в журнале как agent_policy:<id>; потолок 6 решений за процесс",
