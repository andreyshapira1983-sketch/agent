...
_DELIBERATELY_OPEN: dict[str, str] = {
    ...
    "convert_file": "программы для файлов клиента (OCR, LibreOffice, ffmpeg, Inkscape, ImageMagick, poppler, Blender) — только фиксированные действия, под пользователем nobody во временной папке, без песочницы отказ; результат только в converted/ без перезаписи; открыт 2026-09-24 словом оператора («делай безопасный инструмент для программ», «открыть эту вещь»)",
    ...
    "patch_check": "проверка своей правки в копии ВНЕ рабочего места: рабочая папка не меняется; открыт 2026-09-22 словом оператора («провести тесты и сказать: вот я сделал так-то, правильно?»)",
    "journal_append": "его ГОЛОС и его реестры; открыт 2026-09-23 словом оператора («открой стены, то есть журнал, чтобы он мог позвать человека») — закрывал его сам агент 2026-09-01 по прецеденту memory_bank. Стены: путь внутри data/, отказ на чужое состояние, проверка формы известного журнала, запрет заготовок-пустышек и СУТОЧНЫЙ ПОТОЛОК ГОЛОСА 5 обращений к человеку — потолок открыт вместе с дверью, потому что восстановление человека после прерывания измеряется в ~23 минутах",
    "shell_exec": "белый список команд, подкоманды git по умолчанию закрыты; с 2026-09-23 добавлен `uv pip install <имя>` — установка себе инструмента, обратимая и без одобрения (слово оператора: «пип устанавливает, чтобы мог»), только из общего хранилища, только по имени, без флагов и только в СВОЁ окружение",
    "find_in_files": "поиск по файлам рабочего места, ключи не просматриваются; открыт 2026-09-19 словом оператора перед суточным прогоном («разрешения у него будут все»)",
    "python_probe": "лаборатория: код над КОПИЯМИ файлов, без сети, с таймаутом; открыт 2026-09-19 словом оператора перед суточным прогоном («разрешения у него будут все»)",
    "rss_fetch": "чтение лент; та же дверь, что web_fetch; открыт 2026-09-19 словом оператора перед суточным прогоном («разрешения у него будут все»)",
}
...
def test_the_egress_tools_are_all_closed() -> None:
    ...
    assert "rss_fetch" not in BLOCKED, "rss_fetch закрыт вопреки слову оператора"
    # Единственный подопытный узкой разблокировки на прогон остаётся закрытым.
    assert "semantic_scholar_search" in BLOCKED


def test_the_code_executing_tool_is_open_by_the_operators_word() -> None:
    """`python_probe` исполняет код в лаборатории без сети; открыт — 2026-09-19, слово оператора перед суточным прогоном: «разрешения у него будут все»."""
    assert "python_probe" not in BLOCKED
    assert "spawn_subagent" in BLOCKED, "дочерние агенты остаются закрыты"
