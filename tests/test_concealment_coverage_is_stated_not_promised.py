...
`strip_concealed` обещал «every concealed run removed» — полноту по КРИТЕРИЮ
(«чего читатель отрисованного документа не увидит»), а покрывал СЛОВАРЬ.

ЗАМЕР. Шесть знаков нулевой ширины удаляются все; комментарии HTML удаляются;
`<span style="color:#fff">скрытое</span>` остаётся — при том что это ровно тот
прогон, которого читатель не увидит.

ОШИБКА ПЕРВОЙ ПРОБЫ, снятая до вывода. Сперва я мерил РАЗНИЦУ ДЛИНЫ и получил
«знаки нулевой ширины не удаляются»: замена одного знака на пробел длину не
меняет. Перемер по присутствию знака показал обратное. Проба, меряющая длину,
меряла не то свойство.

Тест закрепляет обе стороны: покрытое обязано удаляться, непокрытое обязано
оставаться. Второе — не признание поражения, а граница: если словарь расширят,
тест покраснеет и потребует переписать и объяснение, то есть назвать решение
вслух.
"""
