...
def test_model_usage_limits_payload_labels_zero_as_unlimited():
    payload = ModelUsageLimits().to_dict()

    assert payload["max_calls"] == 0
    assert payload["max_tokens"] == 0
    assert payload["max_cost_units"] == 0
    assert payload["max_calls_label"] == "unlimited"
    assert payload["max_tokens_label"] == "unlimited"
    assert payload["max_cost_units_label"] == "unlimited"
    assert payload["max_calls_enforced"] is False
    assert payload["max_tokens_enforced"] is False
    assert payload["max_cost_units_enforced"] is False


def test_model_usage_limits_payload_labels_enforced_caps():
    payload = ModelUsageLimits(max_calls=2, max_tokens=100, max_cost_units=5).to_dict()

    assert payload["max_calls_label"] == "2"
    assert payload["max_tokens_label"] == "100"
    assert payload["max_cost_units_label"] == "5"
    assert payload["max_calls_enforced"] is True
    assert payload["max_tokens_enforced"] is True
    assert payload["max_cost_units_enforced"] is True
