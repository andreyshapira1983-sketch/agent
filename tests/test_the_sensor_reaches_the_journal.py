...
Тест ниже переписан под эту границу: он стережёт уже не отсутствие
потребителя, а отсутствие ПОДЪЁМА, и покраснеет в день, когда производство
начнёт строить утверждения.
"""
...
def test_nothing_in_production_climbs_the_ladder() -> None:
    """ИЗВЕСТНАЯ ГРАНИЦА, закреплённая нарочно: подъёма нет.

    Наблюдение теперь переживает ход — `core/causal_store.py`. Но выше первой
    ступени не поднимается никто: `CausalClaim` в производстве не строится,
    объяснения не выдвигаются, вмешательства не ставятся. Значит `state_of` на
    живом пути возвращает только `OBSERVED`.

    Когда подъём появится, этот тест покраснеет. Это и есть его работа.
    """
    import re
    ...
    pattern = re.compile(r"CausalClaim\(|is_lesson\(|state_of\(")
    owners = {"causal_lesson.py", "loop_memory_write.py", "causal_store.py"}
    consumers = [
    ...
    assert not consumers, (
        "производство начало подниматься по лестнице — описание этого "
        f"кто-то начал строить утверждения: {consumers}"
    )
