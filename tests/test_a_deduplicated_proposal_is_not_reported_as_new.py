...
СВЕРКА С ПОЛЕМ, форма F-1 «ключ дедупликации»
(docs/audit/FIELD_CHECK_QUEUE.md). Провал, который поле приписывает этой форме:
ключ не содержит различающего поля, два разных предмета схлопываются в один, и
второй теряется молча.

ЗАМЕР 2026-08-24. Из четырёх наших ключей два несут отпечаток содержимого
(`self_split:{target}:{digest}`, `proposed_task:{signature}`), а два — только
путь: `self_apply:{path}:campaign_diagnosis` и
`self_apply:{target}:campaign_doctrine_draft`. Проба: две заявки с одним ключом
и РАЗНЫМ содержимым дают один и тот же id, в ящике остаётся первая.

САМА ДЕДУПЛИКАЦИЯ ЗДЕСЬ НЕ ОСПАРИВАЕТСЯ. Она задокументирована и заведена ради
дела: без неё демон копил повторяющиеся грозди `proposed_task`. Потеря второго
черновика к тому же ограничена сроком жизни заявки.

ЛОЖЬ НЕ ОГРАНИЧЕНА НИЧЕМ, и чинится она. Вызывающий получает id и пишет в
журнал «черновик предложен, N символов» — длину НОВОГО текста при том, что в
ящике лежит старый, и возвращает `doc_draft_proposed:<id>`. Запись говорит о
предложении, которого не было; человек, читающий журнал, видит работу, которой
не случилось.
"""
