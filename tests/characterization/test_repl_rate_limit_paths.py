...
def _run_repl(
    ...
    *,
    ...
    monkeypatch.setattr(main_module, "handle_meta_command", lambda *a, **k: meta)
    monkeypatch.setattr(main_module, "_handle_local_operator_reply", lambda *a, **k: local_reply)
    monkeypatch.setattr(
        main_module, "handle_conversational_operator_input", lambda *a, **k: conversational
    )
    monkeypatch.setattr(
        main_module, "_run_agent_with_budget_guard", lambda agent, **kw: runs.append(kw) or "A"
    )
