...
#: +2 (2026-09-01): PLR0912/PLR0913 у run_campaign и run_paced_campaign — право
#: сменить исчерпанную цель внутри прогона добавило ветку и параметр. Плоские
#: ветки: каждая — отдельный исход выбора (сменили / отказ хартии / падение
#: выбора / лимит смен), и склейка спрятала бы именно то, что надо видеть.
#: +2 (2026-09-01): PLR0915 в agent_tick.py и core/campaign.py — подпись агента:
#: «record_self_stop добавил по 7 операторов в run_campaign и agent_tick.py,
#: которые стояли у самого порога PLR0915. Это плата за обязательный орган
#: ядра, а не мусор — третья и последняя планка».
#: +1 (2026-09-01): TRY004 в tools/journal_append.py — контракт агента (append_tool_design.md)
#: обещает ValueError на не-dict record; ruff предпочёл бы TypeError, но контракт
#: принят дословно и свидетель проверяет именно ValueError. Долг именной.
#: +1 (2026-08-31): SIM103 в core/persistent_memory.py — ворота _looks_like_code
#: двери памяти (авторство агента, принятая дословная редакция). Ruff хочет
#: схлопнуть две последние ветки в `return bool(...)`, но лестница из четырёх
#: явных правил — его формулировка порогов; сжатие спрятало бы третье правило.
#: Долг именной, редакция принята дословно по закону курьера.
_BASELINE_FINDINGS = 86  # 779 → … → 136 → 113 → 114 → 115 → 116 → 118 → 120 → 87 → 86. −1 (2026-09-24, слово оператора): UP042 — ComplexityTier стал StrEnum. −33 (2026-09-24): тест молча не шёл на сервере (ruff не стоял), долг дорос до 129; 42 новых замечания с 18.09 закрыты, база опущена, чтобы выигрыш нельзя было проесть обратно. +1 (2026-08-29): PLR0911 в
#: core/placeholder_text.py — сторож заглушек v2 (авторство агента) собран из
#: слоёв «токен / старый закон / структурный разбор», и каждый слой отвечает
#: своим ранним `return`. Дробить их ради счётчика значило бы прятать ту самую
#: последовательность решений, которую сторож и документирует. Забанковано, а не
#: срезано; долг именной. В тот же день из-под линта выведен `.agent_drafts`
#: (черновики агента давали +70 шума — это объём экспериментов, не рост кода).
#: Партия 1: 174 находки были не
