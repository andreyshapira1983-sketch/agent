...
#: 405 → 406 (2026-08-17): дрейф инструмента, не дерева — на коммите ea6a836,
...
_BASELINE_FINDINGS = 113  # 779 → … → 358 → 265 → 192 → 166 → 159 → 136 → 113. Партия 1: 174 находки были не
...
#: Партия 6: механика без семантики — 21 безопасная автоправка ruff,
#: ISC004/FURB192/PLW1510 (10, каждый ISC004 прочитан: забытых запятых
#: нет), E741 (6). S101 (14) — все до одной сужение типа после настоящей
#: проверки, подавлены по месту с причиной, чтобы правило осталось живым
#: для первого настоящего assert в бою. НЕ трогали BLE001 (124) и S110
#: (42): у каждого подавления должна быть доказуемая причина, верная
#: область try и названное поведение при исключении — это разбор, а не
#: массовая директива.
...
#: Партия 7: BLE001/S110 — 166 мест разобраны поимённо, ноль осталось.
#: Три сужены по-настоящему; два сужения оказались НЕВЕРНЫ и откачены
#: (тест про best-effort реестр и json.dumps в пульсе — узкий except
#: убил бы тик). Остальные подавлены с названным поведением при
#: исключении. Третий критерий — область try — ИЗМЕРЕН, а не объявлен:
#: по 242 подавленным местам медиана тела 6 строк, широких (>=4
...
#: Партия 9 — сложность (PLR09xx), 36 находок на 21 функции. Число не
#: диагноз: у плоской диспетчеризации ветвлений столько, сколько слов в
#: её словаре. Мерилось три вещи — глубина вложенности, доля возвратов,
#: которые являются ранними отказами, и число операторов ниже 4-го
#: уровня. 15 функций оказались плоскими (у `handle_meta_command` 94
#: возврата и все 94 — стражи) и подавлены с этим замером по месту.
#: 7 остались открытыми, потому что замер против них; худшая —
#: `verifier_core.verify`, глубина 15 и 127 операторов ниже 4-го уровня,
#: записана как MIR-103 и НЕ переписана: замер обосновывает «это трудно
#: читать», но не конкретное разбиение.
