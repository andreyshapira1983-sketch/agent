...
def test_a_window_of_a_large_book_is_readable(workspace: Path):
    """Замер 2026-09-19 (опыт с библиотекой книг): findstr нашёл строку 14849 в
    Erickson_Algorithms.txt (1.1 МБ), а окно 14820-14880 отказало «File too
    large»; треть книг библиотеки крупнее 1 МБ. Целиком — по-прежнему нельзя."""
    lines = [f"line {n} " + "x" * 80 for n in range(1, 15001)]
    (workspace / "book.txt").write_text("\n".join(lines), encoding="utf-8")
    assert (workspace / "book.txt").stat().st_size > MAX_BYTES
    tool = _tool(workspace)

    out = tool.run(path="book.txt", start_line=14849, end_line=14850)
    assert "14849: line 14849 " in out and "of 15000]" in out

    with pytest.raises(ValueError, match="start_line/end_line"):
        tool.run(path="book.txt")
