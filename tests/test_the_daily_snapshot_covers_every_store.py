...
многодневный отказ Atlassian, третья часть: восстановление для одного, когда
нужно для многих.

ЗАМЕР 2026-08-25: 19 живых хранилищ из 24 не имели ни одной копии, включая
`budget_ledger` (деньги) и `approval_inbox` (полномочия); `data/` исключён из
git, версионный контроль не запасной путь. Решение оператора того же дня:
снимок раз в сутки, хранить семь.

Тесты закрепляют ровно то, что делает снимок ценным, и ровно те границы, за
которыми он стал бы опасен: покрытие ВСЕХ хранилищ (иначе повторяется исходная
ошибка прибора — перебор не того множества), идемпотентность по суткам,
прополка только СВОИХ поколений и неприкосновенность живого состояния.
"""
