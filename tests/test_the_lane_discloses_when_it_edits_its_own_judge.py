...
MIR-139. Внешнее утверждение (препринт 27 июля 2026, «Self-Authored Verification
Is Unreliable…»): когда агент меняет политику И собственные тесты одним актом,
самооценка остаётся почти идеальной, а настоящее качество падает. Разделение
проведено полем не по «трогать тесты», а по «менять политику и её судью
одновременно» — и это важно, потому что честная работа тесты трогает: Stage A
обязан сперва написать ПАДАЮЩИЙ приёмочный тест, а починки правят существующие.

ЗАМЕР 2026-08-24. `classify_patch_risk` смотрит ТОЛЬКО на пути: разрешён и не
запрещён — значит «низкий риск». Содержимого он не видит вовсе, поэтому патч,
который правит `core/` и одновременно вырезает проверки из `tests/`, проходит
как низкорисковый, а зелёный прогон, который он затем даёт, предъявляется
человеку как доказательство.

ЧТО ЗДЕСЬ ДЕЛАЕТСЯ, А ЧТО НЕТ. Не вводится второй судья и не ужесточается
собственный набор тестов — анти-требование записи прямо запрещает и то, и
другое: более строгий самописный аршин остаётся самописным. Решение оставлено
там, где оно и живёт по §9 — у человека в ящике одобрений. Закрывается ровно то,
чего человеку не хватает: факт, что судья изменился, и в какую сторону.
"""
