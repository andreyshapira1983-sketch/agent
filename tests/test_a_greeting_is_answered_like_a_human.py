...
def test_the_politeness_dictionary_is_pinned_against_rot() -> None:
    """Двухуровневый триггер (уточнение Codex: хэш ловит ЛЮБУЮ правку, не
    гниль). Уровень 1 — РОСТ числа формул: событие AIML-диагноза, главный
    вектор гнили тридцати лет; отдельное сообщение. Уровень 2 — любая иная
    правка (сокращение, перестановка, починка): явный акт с обновлением
    пина, диагнозом не является. Правило «второе расширение подряд — орган
    меняется на классификатор» остаётся процессным (в записке): читать
    историю коммитов тест не берётся — счётчик истории сам стал бы
    «специальной метрикой» из его же списка векторов гнили."""
    import hashlib

    from core.conversation_contract import _SMALL_TALK_RE

    alternatives = _SMALL_TALK_RE.pattern.count("|") + 1
    assert alternatives <= 24, (
        f"словарь формул ВЫРОС ({alternatives} > 24) — событие AIML-диагноза "
        "(CONVERSATION_ORGAN_DESIGN.md): расширение требует явного решения, "
        "а второе подряд означает переход к классификатору"
    )
    ...
    assert digest == (
        ...
        "словарь формул изменился (не ростом) — это явный акт: обнови пин "
        "тем же коммитом с причиной: " + _SMALL_TALK_RE.pattern
    )
