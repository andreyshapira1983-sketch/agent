...
Both operator triggers journal here:
...
strictly best-effort: it never raises and never blocks the command.
"""
...
def record_self_build_episode(agent: Any, *, kind: str, result: dict[str, Any]) -> bool:
    ...
    failure (no store, bad record, disk error) is swallowed so the operator
    command is never broken by memory journaling.
    """
    try:
