...
> **Документы проекта (единственный индекс — этот раздел).**
> Новый документ заводится только под роль, которой нет ниже; иначе — дополняем существующий.
>
> | Документ | Роль |
> | --- | --- |
> | этот раздел README | *что сейчас происходит* — живое состояние + журнал |
> | [`AGENT_DOCTRINE.md`](AGENT_DOCTRINE.md) | *по каким правилам* агент живёт (принципы) |
> | [`архитектура автономного Агента.txt`](архитектура%20автономного%20Агента.txt) | *как устроено* (архитектура, целевая модель) |
> | [`docs/AGENT_ANATOMY.md`](docs/AGENT_ANATOMY.md) | *что реально подключено* — read-only карта анатомии: wired-live / manual-only / duplicated (TD-029) |
> | остальной README | *справочник* по реализованным слоям |
...
| Метрика | Значение |
| --- | --- |
| Тесты | **2825 / 2825 зелёные** (hermetic, zero network) |
| Покрытие core+tools | **~89–90%** |
| Git HEAD | `5e3878c` — close coverage gaps + drop throwaway scripts |

### Стабильность слоёв (по живым прогонам, 25 мая – 4 июня)

| Слой (task kind) | Прогонов | Стабильность | Примечание |
| --- | --- | --- | --- |
| `status` | 83 | **100%** | — |
| `propose` | 17 | **100%** | 10 свежих живых прогонов, все done |
| `tests` | 55 | **96.4%** | сбои — не свои баги |
| `learn` | 54 | **92.6%** | skipped = ротация корня/бюджет, не ошибка |
| `goal` | 14 | **100%** | +10 живых прогонов (`проверь покрытие тестов`), все done, ответ честный (без галлюцинаций) |

**Все «ошибки» в логах = сработавшие защиты** (approval-гейт, песочница workspace,
бюджет-гавернор) либо внешний мир (SSL), либо уже исправленный `file_not_found`
(0 повторов в свежих логах). Критерий стабильности слоя: **≥10 живых прогонов
без regression + покрытие ≥85%**.

### Журнал значимых изменений

- **2026-06-04** — слой `goal` доведён до 100% (14 живых прогонов): цель
  `проверь покрытие тестов` стабильно отрабатывает, ответ честный — агент
  диагностирует отсутствие `pytest-cov` и не выдумывает процент покрытия.
- **2026-06-04** — закрыты дыры покрытия (+36 тестов: `recover_stuck`,
  `_apply_staleness`, `_merge_unverified`, integrity-check, `user_summary`,
  unknown-task-kind); удалены 4 throwaway-скрипта.
- **2026-06-04** — семантический dedup для `propose` (token-Jaccard);
  убрал ~43% переформулированных дублей в approval inbox.
- **2026-06-03** — добавлен task kind `propose` (предложения без исполнения).
- **2026-06-02** — Layer 4 User Profile; ранее Layer 1–3
  (ClarificationPolicy, Checkpoint/Resume, Prompt Registry).
...
| §14.3i Conversational Operator + Programming Planning Layer | [`core/operator_intent.py`](core/operator_intent.py) + `:operator-check` / `:coding-readiness` / `:source-review-plan` / `:implementation-plan` / `:patch-proposal-plan` |
...
3. Pick LLM provider via env vars:
  ...
  per-session caps stop the next model call before it is sent:
   ...
   sessions. Usage is stored in `data/budget_ledger.jsonl`; zero means
   "not enforced". You can set limits either through PowerShell environment
   variables or by copying `config/budget_limits.example.json` to the local,
   git-ignored `config/budget_limits.json`:
   ```powershell
   ...
   ```
   Environment variables override `config/budget_limits.json` for the current
   process. `:autonomy-readiness` stays `limited` until at least one persistent
   hour/day budget limit is configured.
...
- **Interactive** (no `--ask`) — a session with Working Memory **and**
  ...
  `:smart-memory`, `:memory-consolidate`,
  `:ingest-source`, `:ingest-project`, `:clear`, `:remember`, `:forget`,
  `:operator-check`, `:coding-readiness`, `:source-review-plan`,
  `:implementation-plan`, `:patch-proposal-plan`, `:capability-request`, `:models`,
  `:model-registry-audit`, `:architecture-audit`, `:hygiene`, `:rollback`,
  `:help`, `:quit` for control.
...
> :smart-memory                       # episodic/procedural/consolidation memory status
> :memory-consolidate                 # force a local consolidation report
...
> :coding-readiness                   # safe programming-task readiness report
> :source-review-plan "compare core/operator_intent.py and main.py"
                                      # compare explicit local sources already in registry
> :implementation-plan "implement operator routing in core/operator_intent.py and main.py"
                                      # local source-backed implementation plan
> :patch-proposal-plan "fix operator routing in core/operator_intent.py and main.py"
                                      # read-only patch proposal plan; no file write
...
> :capability-request "Следи за важными письмами" --submit
                                      # add the proposed capability boundary to approval inbox
...
## Smart Experience Memory (MVP-18.3)

Implemented in [`core/smart_memory.py`](core/smart_memory.py) and wired into
[`core/loop.py`](core/loop.py) when the interactive agent has persistent state
enabled.

This is operational learning, not model retraining:

- **Episodic memory** records completed cycles as compact episodes: goal,
  question, outcome, summary, tools used, source labels, verifier counts and
  replan exhaustion.
- **Procedural memory** promotes successful tool workflows into reusable
  procedures. A repeated `file_read -> verify -> answer` pattern becomes a
  small workflow with confidence and source episode links.
- **Memory consolidation** periodically links episodes to procedures and writes
  an auditable report with active / needs-review / obsolete procedure groups.

Default local files:
```
data/episodic_memory.jsonl
data/procedural_memory.jsonl
data/memory_consolidation.jsonl
```

How it is connected:

- before planning, relevant procedures and episodes are injected as
  `<agent_experience_memory>`;
- after responding, the loop writes one episode, updates one matching procedure
  when the cycle succeeded with tools, and writes a consolidation report;
- the final answer does not cite experience memory as factual evidence. It is
  planner guidance only, so old experience cannot masquerade as a verified
  source.

REPL commands:
```
:smart-memory [--json]        show episode/procedure/consolidation counts
:memory-consolidate [--json]  force a consolidation report without LLM calls
```

Logged smart-memory events:
```
smart_memory_load          session start counts and paths
experience_memory_inject   selected procedures/episodes for planning
episodic_memory_write      completed cycle recorded as an episode
procedural_memory_update   reusable workflow created/updated/skipped
memory_consolidation       episodes linked to procedures
smart_memory_error         best-effort guard if a local state file is malformed
```
...
| Module | Job |
| --- | --- |
| [`secret_scanner.py`](core/secret_scanner.py) | Single source of truth for credential detection. Regex rules (OpenAI / Anthropic / GitHub / HuggingFace / AWS / Bearer / PEM / JWT / Telegram bot token / MongoDB URI / `KEY=VALUE` assignments) + keyword rules (`password`, `api_key`, `Authorization:`, …). Every consumer (Memory Write Policy, Redaction, Data Classifier, the loop) asks **this** module — nobody rolls their own patterns. |
| [`dlp.py`](core/dlp.py) | Detects sensitive PII markers (email, SSN, international `+phone`, Russian ИНН 10/12-digit, СНИЛС `XXX-XXX-XXX XX`) and returns typed findings for logs, memory and prompt-boundary policy. |
...
### 14.3i — Conversational Operator + Programming Planning Layer

This slice starts replacing command memory with normal operator language.
Common owner and programming-planning requests are routed locally before the
LLM is called:
  ...
  - "Проверь, готов ли ты к безопасной программной задаче" routes to
    `:coding-readiness`.
  - "Сравни / прочитай эти исходники" routes to `:source-review-plan`.
  - "Составь план реализации" routes to `:implementation-plan`.
  - "Составь patch proposal" routes to `:patch-proposal-plan`.

The operator layer is intentionally read-only. The system digest combines
architecture audit, source registry counts, persistent memory count, approval
status, queue / scheduler status, model usage and budget windows, then proposes
next actions. Programming requests produce local planning reports: source
review, implementation plan, or patch proposal. They do not write files, run
shell commands, enable effects, or promote memory by themselves.

The intent vocabulary currently includes `safe_self_check`, `capability_check`,
`current_gaps_check`, `weakness_finder`, `next_safe_test`,
`programming_readiness`, `source_review_plan`, `implementation_plan`, and
`patch_proposal`. This keeps the user-facing surface conversational while
preserving the existing auditable command handlers underneath.
...
This slice gives the runtime memory between launches:
  ...
  - `TaskQueueStore` persists autonomous tasks in
    `data/runtime_tasks.jsonl`. Includes `recover_stuck(timeout_minutes=30)` to
    reset stale "running" tasks back to "pending" after the given timeout, so
    no task is permanently stuck after a crash or process kill.
...
This slice moves from "the owner manually wires every capability" toward
"the agent proposes missing capabilities by itself". It still does not activate
connectors or launch real subagents. It only creates a bounded
`CapabilityRequest`:

  - `capability_type`: `telegram`, `email`, `upwork`, `subagent`,
    `web_monitor`, `file_workspace_access`, `long_work_session`,
    `model_tier_upgrade`, `persistent_memory_write`, or `unknown`.
  - `why_needed` and `requested_access`: why this capability is needed for the
    natural-language goal.
  - `read_scope`, `write_scope`, `forbidden_actions`: explicit access boundary.
  - `budget_limits`: hard operator-facing limits such as messages/day,
    model calls, web fetches or jobs/day.
  - `human_risk_summary` and `fallback_if_denied`: what can go wrong and what
    the agent will do if the owner says no.
...
Natural phrases such as "сам сообщал мне, когда нужно решение" route locally to
the same request layer before the LLM planner is called. `--submit` places the
proposal into the Approval Inbox with operation `capability_request`; it does
not activate Telegram, email, Upwork, premium models, memory writes or long
sessions. Activation is a later layer and must consume an approved boundary.
...
| Secret Scanner (§7) | [`tests/test_secret_scanner.py`](tests/test_secret_scanner.py) | parametrised regex hits for OpenAI / Anthropic / GitHub / HuggingFace / AWS / Bearer / PEM / JWT / Telegram bot token / MongoDB URI / `KEY=VALUE` assignment / span consistency / Anthropic-vs-OpenAI disambiguation / case-insensitive keywords / **combined regex+keyword signals both surface in audit reasons** |
| DLP PII Scanner (§7) | [`tests/test_dlp.py`](tests/test_dlp.py) | detects email / SSN / international `+phone` / Russian ИНН (10 or 12 digits) / СНИЛС (`XXX-XXX-XXX XX`), reports unique sorted markers, and stays silent on normal technical text |
...
| Conversational Operator + Programming Planning Layer (MVP-14.3i) | [`tests/test_operator_intent.py`](tests/test_operator_intent.py), [`tests/test_cli.py`](tests/test_cli.py) | natural-language project/model/budget/approval/urgent/next-action/autonomy-readiness/coding-readiness/source-review/implementation-plan/patch-proposal requests route to local handlers before LLM calls / intent kinds include `safe_self_check`, `capability_check`, `current_gaps_check`, `weakness_finder`, `next_safe_test`, `programming_readiness`, `source_review_plan`, `implementation_plan`, `patch_proposal` / CLI includes `:operator-check`, `:operator-budget`, `:urgent-status`, `:next-actions`, `:autonomy-readiness`, `:coding-readiness`, `:source-review-plan`, `:implementation-plan`, `:patch-proposal-plan` text + JSON digests where supported |
...
Explicitly deferred until later MVPs: broad multi-file planner/action workflows
with effects. Safe explicit read-only multi-file review exists for
user-mentioned workspace paths, but PDF/DOCX, large-file chunking,
multi-file search / indexing, embeddings / vector
...
│   ├── test_smart_memory.py          # episodic/procedural/consolidation memory: 8 cases
│   ├── test_approval.py              # approval gate (unit + 7 acceptance): 43 cases
│   ├── test_secret_scanner.py        # regex + keyword detection: 17 cases (+ JWT, Telegram, MongoDB)
...
│   ├── smart_memory.py               # Episodic / procedural / consolidation memory (§4 MVP-18.3)
