...
A terminal citation refusal records a failed episode and blocked delivery,
not a successful answer. Verification counts and the synthesizer's declaration
remain available for auditing, but the summary and quality score explicitly
refer to the rejected draft, not the delivered refusal. Existing memory is not
rewritten. Provider-health fallback is disclosed alongside ordinary failover;
it does not imply that the skipped model was called.
