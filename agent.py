...
def build_agent(
    ...
    # ── Дешёвый LLM для фонового цикла (не жрёт бюджет) ─────────────────────
    _bg_model = os.environ.get('BACKGROUND_MODEL', 'gpt-4o-mini').strip()
    _bg_client = None
    if openai_key and _bg_model:
        try:
            from llm.openai_client import OpenAIClient
            _bg_client = OpenAIClient(
                api_key=openai_key,
                model=_bg_model,
                max_tokens=1024,   # фоновые задачи — короткие ответы
                temperature=0.5,
                monitoring=monitoring,
            )
            monitoring.info(
                f"Фоновый LLM: {_bg_model} (max_tokens=1024) — экономный режим",
                source="agent",
            )
        except Exception as _bg_err:
            monitoring.warning(f"Не удалось создать фоновый LLM: {_bg_err}", source="agent")
            _bg_client = _openai_client  # fallback на основной
    ...
    # Фоновый LLM для автономных циклов — дешёвый, не трогает gpt-5.1
    if _bg_client and _bg_client is not _openai_client:
        if hasattr(loop, 'background_llm'):
            loop.background_llm = _bg_client
        monitoring.info(
            f"Фоновый цикл использует {_bg_model} вместо основного LLM → экономия ~95%",
            source="agent",
        )
...
class Agent:
    ...
    def start_loop(self, goal: str, cycle_delay: float = 60.0,
...
Примеры:
  ...
  python agent.py --model gpt-5.1 "сложная задача"
    ...
    if args.bot:
        ...
        bot_delay = float(os.environ.get("BOT_LOOP_DELAY", "60"))
