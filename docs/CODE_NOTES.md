...
## R9 addendum 2: a method boundary is not a retraction (2026-09-19, external exam)

One task (sums by name → report.json) ran ten times: nine answers took
self_contradiction and not one episode became experience, so the memory arm
never saw a past run. All nine caveats were a boundary of METHOD — «прочитано
не было — подтверждён только факт записи (87 байт)», «не перечитывал», «не
подтверждено отдельным чтением», «область доказательств ограничена data.csv».
The vocabulary moved to `_BOUNDARY_RE` and learned word-order variants,
reading-as-method, «отдельно/косвенно/напрямую», evidence scope and «подтверждён
/ известны только…» (such a line affirms its subjects). Two false SUBJECTS
closed: a digits-only date is not a SHA (a hex letter is required), a path ends
at its extension (`report.json` is not inside `report.json.bak.…`), and a
denied subject must be a subject OF the line, not a substring of it. Measured
on every banked exam answer: experiment 9/10 → 0/10, all runs 84/201 → 47/201.
Open: the remaining 47 are mostly «another attribute of the same file»
(«не подтверждено, что после правки все 5 тестов пройдут» vs «5 тестов» in
Facts); telling them from «не доказано, что X — коммит этого прогона» (pinned)
needs comparing judgments, not subjects. Witness:
`tests/test_a_method_boundary_is_not_a_contradiction.py`.
...
## A read is not the end of the turn; a worker thread is still inside the run

Measured 2026-09-19 with an external exam of 30 code-checked tasks
(`Downloads/agent/exam`, outside this repository). Three repairs came out of it.

**The planner's own example taught an unresolvable reference.** The prompt
showed the first step as `{{step:0.output}}`, while steps are numbered from 1.
Six unresolved references across five failed tasks traced to that line; after
the example became `{{step:1.output}}` there were none (`core/planner_prompt.py`).

**The attempt loop answered on the first result.** A successful batch ended the
turn, and a new plan was drawn only after a failure, so "read — compute or edit —
write" could not happen inside one turn: the arguments were fixed before the read.
Step references were a patch on that hole and could only carry output unchanged.
`core/observation_round.py` hands the planner the real outputs after a successful
batch; an empty plan ends the turn, rounds share `max_total_replans`. The
completion contract is deliberately not the trigger: on the same tasks it missed
sum.txt and todo.txt and demanded an edit to a test file the task forbade touching.
Opt-in (`AGENT_OBSERVE_BEFORE_ANSWER=1`) until measured wider. On the nine affected
tasks, three attempts each: 17 of 27 passed, against 0 of 9 before; all three
repair tasks passed every time. Advice blocks tagged `<observed_results>` are
inserted verbatim (`core/replan.py`): the indented advice path had dropped blank
lines and shifted file content, which the new test caught.

**Parallel reads lost the run's restrictions.** `executor.submit` does not carry
a `ContextVar`, so a plan of two or more reads ran in worker threads that could
not see `run_restrictions`: tools closed for the run executed anyway (the exam's
closed-network task fetched pypi.org; a closed-file quiz read the files). Plans
with an effect were not affected — they run in order on the calling thread.
Each worker now runs in `copy_context()` (`core/loop_step_execution.py`).
Witness: `tests/test_a_parallel_read_keeps_the_runs_restrictions.py`, red on the
previous code, green now. The observation round's witness is
`tests/test_a_read_is_not_the_end_of_the_turn.py`.

## One unmatched citation no longer takes the verified answer with it

Operator's word 2026-09-19, «исправь фильтр цитат», revising R4 (2026-08-13).
R4 made a fabricated citation terminal: any `cited_but_unmatched` chunk replaced
the whole answer with «Черновик ответа не отправлен». Measured on the external
exam: four withheld answers out of four carried exactly ONE such chunk, never the
conclusion, beside up to four verified claims — the operator received a service
line instead of «the file does not exist» or «I cannot verify this offline».

`core/unsupported_claims.py::_excise_fabricated` now cuts only the claims whose
citation did not match, together with a source-list entry cited by nothing else,
and ships the rest with a notice (`EXCISION_PREFIXES`, kept by the human printer in
`core/answer_format.py`). What R4 protects still holds: an unmatched citation never
reaches the operator. The whole-answer refusal remains when nothing verified would
be left (R4's own «36 × 3 = 108» case) and when a claim cannot be located exactly
in the text — nothing is cut by guesswork. The episode carries `citation_excised`,
added to `DISQUALIFYING_DEFECT_SIGNALS`: the answer is delivered but never becomes
experience. Witnesses: `tests/test_rejected_draft_is_not_a_completed_answer.py`
(withheld path on a fully fabricated draft, excision path on one bad claim) and
`tests/test_fabricated_citations_are_terminal.py` (unchanged, green). Exam tasks
19, 22, 23, three attempts each: 9 of 9 after the change.

**Addendum, same day: the episode lost the tools of earlier rounds.** With the
observation round the last plan is often empty («done»), and `tools_used` was read
from that plan alone — a turn that read and wrote a file banked an empty list, the
very list procedure learning reads. `core/smart_memory.py::episode_tools` keeps the
last plan and appends whatever executed earlier; without rounds everything executed
is already in the last plan, so nothing changes. Witness:
`tests/test_a_read_is_not_the_end_of_the_turn.py::test_the_episode_names_every_tool_the_turn_ran`.

## The lab computes over task files; a command's result travels as its output

Measured 2026-09-19 on the external exam. Sums of 40–60 numbers were done "in the
head" and came out wrong (411 for 8009, 10231 for 9912); twice the agent wrote a
round 10000 and admitted in the same answer that nothing was computed. The lab
(`tools/python_probe.py`) could not see task files: its cwd is an empty temp dir.
`inputs` now copies named workspace files into that dir — relative paths inside
the workspace only, size- and count-capped, copies so nothing is written back; the
planner is told to compute such numbers there, never to estimate.

The computed value then died in transit: `{{step:1.output}}` delivered the probe's
whole result dict and `file_write` refused it. `core/step_references.py::_delivered`
carries a command's stdout; a command that failed, timed out or was truncated is
not a result and the reference fails with the reason. Other outputs keep their
type. Witnesses: `tests/test_the_lab_computes_over_task_files.py`,
`tests/test_a_command_result_reaches_the_next_step.py`.

**Addendum: a lab that could not see its file gives no result; a clean write ends
the turn.** Same exam, task «recount the sum», second turn: the probe was run
without `inputs`, printed «exists: False», and that line went into sum.txt. The
probe now reports `missing_inputs` — workspace files its code names but was not
given — and `_delivered` refuses such output as a result, naming the files to add.
Separately, after a correct write the observation round re-ran the same probe and
write until the attempt budget ran out; `observation_round._effect_completed_cleanly`
ends the turn when a batch changed something and no step in it failed. A failed
step beside the write (red tests after an edit) still gets its round.

**Addendum: the stop rule took away the round that shows an unseen write.** Within
the hour the same exam task («totals per name») regressed to 0 of 3: the probe
printed a debug line and the JSON, `{{step:3.output}}` carried both into
report.json, the write succeeded, and «a clean effect ends the turn» removed the
round in which the planner would have seen it. A write whose content arrived by
reference was never seen by the planner, so it now keeps its observation round;
the reference is read from the planner's own sources, because a step's arguments
are resolved in place before execution. Witness: the third case of
`test_a_clean_write_ends_the_turn_but_a_failed_step_does_not`.

**Addendum: the round must SHOW the unseen write, and the probe must print only
the file.** Episodic experiment 2026-09-19 (the same task five times in one
memory): twice the sums were right and report.json still failed to parse — the
probe printed «columns: …/row_count: 60» or «rows: 60» beside the JSON, the
reference carried the whole stdout, and the observation round, which saw the
content only as an escaped `"content": "{…}\nrows: 60\n"` inside the argument
line, answered «written exactly the required object». One of those episodes
banked outcome=success, usage_eligible=True and credited the procedure — a failed
run admitted as experience. `format_observations` now prints every written
content verbatim between `<<<`/`>>>` with its path and length, and the planner
prompt says the whole stdout is carried, so a probe feeding a file prints the
file and nothing else. Witness:
`test_the_round_shows_what_a_reference_wrote_verbatim`.

**Addendum: copy from the outputs, compute in the lab.** Same experiment, next
run: the round's own text said «compute values from the outputs above yourself»,
against the prompt's «COMPUTE, never estimate». Five of ten runs read data.csv,
carried the raw CSV into report.json, then added sixty numbers in the head on
the observation round and wrote wrong sums. The round now says a value may be
COPIED from the outputs, and anything that must be computed goes to python_probe.

**Addendum: the lab's evidence carries every fact of its outcome.** True claims —
«the script read data.csv», «missing_inputs: []», «output not truncated» — took
`[claim-refuted]` (`cited_literal_absent`): the excerpt held only exit_code,
timed_out, stdout and stderr, so the literal gate read the other outcome fields
as invented. content_refuted is disqualifying, and the correct episodes lost
their place in experience. `_python_probe_evidence` now adds inputs,
missing_inputs, stdout_truncated and duration_ms; both file lists name only files
that exist in the workspace, and the code still stays out. Witness:
`test_true_facts_of_a_run_are_not_refuted`.

**Addendum: «no errors» is not the absence of the file that was read.** With the
lab's evidence complete, the next run's true claims fell to another gate:
«Скрипт завершился с кодом 0, ошибок нет, входной файл `data.csv` найден». The
«нет» made the whole chunk an absence claim, `absence_subjects` took every named
subject of the chunk, found `data.csv` and `exit_code` in the evidence and ruled
absence_refuted_by_evidence. `absence_subjects` now skips the parts of a
compound sentence (split at , ; —) that themselves assert presence or a done
action («найден», «прочитал», «завершился», found, read, exists) and carry no
absence marker. The pinned compound «атрибут отсутствует, поэтому код с
`itertools.batched` …» keeps its subject: that part asserts nothing present.
Witnesses: `test_no_errors_does_not_make_the_read_file_absent`,
`test_an_absence_beside_a_presence_is_still_judged`.

**Addendum: a place searched is not the thing absent; the runtime block is citable
field by field.** Next run, three more ways a correct episode lost experience.
(1) «ошибок в `stderr` нет» — `stderr` became the absence subject and the
evidence contains «stderr:». A subject right after «в/во/in» is the PLACE of the
search and is presupposed to exist; `_SEARCH_PLACE_RE` drops it, what was sought
is still judged («В `report.py` нет функции `render_total`» stays refutable).
(2) «Совпадение 99 байт не проверялось побайтово» — a method boundary; «побайтов»
joins `_BOUNDARY_RE`. (3) The `<runtime_self>` block showed run_id, trace_id,
session_id, every store and durable_writes, and the output contract says «cite
them as [runtime:<field>]», but `runtime_evidence_pool` knew only the five
process facts, so «(run_id=…) [runtime:run_id]» was excised as a fabricated
citation — citation_excised is disqualifying. The block now records what it
showed (`runtime_self.shown_facts`, keyed by run_id) and the pool offers those
fields in the same run only; another run's block cannot be cited. Witnesses:
`test_every_field_the_block_shows_can_be_cited_in_that_run`,
`test_another_runs_block_cannot_be_cited`.

**Addendum: the answer stops narrating the tools, and a read-window header is not
a fact.** Sixth run of the same experiment: 10/10 correct, two episodes admitted
(both truly correct), the procedure earned its first honest success. What still
cost correct episodes their place was padding — the experiment's encoding, the
interpreter, durations, byte counts, run ids — each one more claim to prove. The
output contract now keeps Facts to what the request needs. And the only
long-term record the run produced was «[data.csv lines 1-20 of 61]»: a file_read
window header, stored at confidence 0.85, which outlived the data it described
and surfaced next run as «memory says 61 lines», blocking a correct episode.
`_looks_like_code_fragment` now refuses read-window headers and sentences cut on
«, : ; — ( →». «не сверялся/не сверен» joins the method-boundary vocabulary.
Witness: `test_a_read_window_header_and_a_cut_tail_are_not_facts`.

## A library of books is not the agent's own repository (2026-09-19, library experiment)

First run of `exam/library_experiment.py` (twelve lookups over ~100 text books,
113 MB). Two defects surfaced that no repo-shaped test could show.

**The verifier route fired on a quotation.** «Чья цитата стоит вторым
эпиграфом» matched the stem «цитат» in `CONFIDENCE_EVIDENCE_DIAGNOSTIC_TERMS`,
and `LLMPlanner` injected reads of core/verifier.py and three tests into every
attempt — files that do not exist in a folder of books. Four reads into nothing
per attempt; the question went unanswered in both arms. The route (injection and
prompt directive) now fires only where those files exist
(`LLMPlanner._confidence_sources_here`); in the agent's own repository nothing
changes, and an unknown root keeps the old behaviour. The three planner tests
that pinned the route now build a workspace that has the files. Witness:
`test_a_quotation_in_a_book_does_not_send_the_planner_to_its_own_verifier`.

**A window of a large file was refused.** findstr found Lao-Tzu at line 14849 of
Erickson_Algorithms.txt (1.1 MB); `file_read(start_line=14820, end_line=14880)`
answered «File too large». A third of the library is over 1 MB. The 1 MB cap now
guards WHOLE-file reads only (and its message says how to read a window); a
start_line/end_line window works up to 50 MB. Witness:
`test_a_window_of_a_large_book_is_readable`.

**Addendum: a book is cut into sentences, not lines.** The same run's long-term
memory held 30 records and not one was knowledge: «320: Consequence is that
(fig 1.5)…», «5: Michaelmas term 2021/22», «7: 1 Introduction 7», «5304:
eβ(ℏ2k2/2m−µ) + 1» — each at confidence 0.85, some recalled into later answers
seven times. `ClaimExtractor.extract` split excerpts LINE BY LINE: a PDF-derived
book breaks every sentence across lines and a file_read window numbers each
line, so every title, contents line and formula shard became a «fact». Extraction
now goes through `_statements`: the window header and line numbers are stripped,
wrapped lines are rejoined (a continuation starts lower-case), and only a
finished sentence or a list item becomes a claim candidate. `_sentences` is left
for the injection guard's spans. Witness:
`test_a_book_window_yields_sentences_not_its_lines`.

**Addendum: the lab copies a named workspace file by itself, and never a key.**
Second library run: the planner rightly chose python_probe for «how many PAGE
markers in Tong_Cosmology.txt», but forgot `inputs`; the experiment died on
FileNotFoundError, the observation round read that as «wrong path», and the
attempt budget ran out. `missing_inputs` (this morning) only complained, and
the model keeps forgetting. A workspace file the code names is now copied
automatically (`auto_inputs`), read-only and within the same limits; what does
not fit stays in `missing_inputs` and is still not a result. The same pass
found a hole: `inputs` would copy `.env` into the experiment, which file_read
refuses to read. `_copy_inputs` now applies file_read's credential gate, and an
auto copy never takes a credential file. Witnesses:
`test_a_task_file_named_but_not_given_is_copied_by_itself`,
`test_a_file_over_the_limit_is_still_named_as_missing`,
`test_the_lab_never_copies_a_credential_file`.

**Addendum: what the turn found out is remembered; a search tool exists.** Two
library runs, fairly scored: 10 and 9 of 12 answered with memory, 8 and 7
without — experience helps; but recalled from memory with the files closed, 2
of 12. Long-term memory held source sentences, chance ones for the question,
while the answer the turn established («section 2.2 of Judson is The Division
Algorithm» — a heading, not a sentence) existed only as the turn's conclusion and
was never kept. `core/learned_conclusion.py`: when an episode is ADMITTED to
experience (`decide_usage_eligibility` — success, achieved, verified claims, no
disqualifying signal) and rests on a source of that turn, its Conclusion
(question → conclusion → sources, citations stripped; «not found / don't know»
excluded) goes to long-term memory through the ordinary `remember` door —
consent tag, dedup, echo, audit freeze all apply
(`AgentLoopMemoryWrite._remember_conclusion`). Witness:
`tests/test_a_verified_conclusion_is_remembered.py`.

Most remaining misses were «this book is not in the library» while it was:
the only search was findstr through shell_exec — `/c "===== PAGE"` failed on
syntax, `/s` without a file mask found nothing, and the empty output read as
absence. `tools/find_in_files.py`: files by name glob and lines by text,
recursive, cross-platform, `path:line: text` feeding file_read windows; a
negative answer says how many files were searched; credential files are never
searched. Closed on the unattended path until an operator opens it. Witness:
`tests/test_find_in_files.py`.

**Addendum: lines are not occurrences.** Third library run: `find_in_files`
reported «125 matches» for «entropy» — 125 matching LINES — and the agent gave
that as the number of occurrences (131). The header now names both:
«N matching lines (M occurrences)».

**Addendum: a file the lab counted was observed.** Third library run: all three
counting questions («how many times does entropy occur in
Tong_StatisticalPhysics.txt») were answered correctly and lost their place in
experience — `completion_obligation` read «was the named file observed» off the
artifact LABELS, and a python_probe label is the head of its code
(«python_probe:import re»), so the duty stayed `silently_missing`, completion
fell to `partially_achieved`, and the conclusion was never remembered.
`_observed_blob` now counts a file the lab was given (`inputs`) and a file the
search found lines in; the search's own label carries the QUERY and is not an
observation. find_in_files joins `_OBSERVING_TOOLS`. Two smaller finds of the
same run: a search given a FILE path answered «Directory not found» (now it
searches inside that file), and «не удалось прочитать файл при повторной
попытке» — a failed attempt, a method boundary — was read as a retraction.
Witnesses: `tests/test_a_lab_count_observes_its_file.py`,
`test_a_file_path_searches_inside_that_file`.

**Addendum: an admitted conclusion is not a verified one.** Fourth library run:
11 of 12 stored conclusions right, one wrong — «the Lao-Tzu epigraph is on page
326» (327: the agent read the text before a PAGE marker as the previous page).
The episode passed every check: its claims rest on what the turn read, nothing
contradicted, nothing refuted. Those checks establish grounding, not
correctness. The tag `verified-episode` promised more than was checked; it is
now `admitted-episode`. The exam's library experiment compares every stored
conclusion with the question's key and prints the wrong ones each run.

**Addendum: a remembered answer is re-checked at its source, and a correction
replaces it.** After the fourth run left «page 326» in memory: (1) the planner
prompt now says a remembered conclusion or past episode is a HYPOTHESIS — when
the source it names is readable this turn, plan one targeted step that confirms
the value there; when it is not, answer from memory and say so; the synthesizer
prompt says the evidence wins and the answer names a remembered value that was
wrong. (2) A corrected conclusion used to be refused as a near-duplicate of the
wrong one, fixing the error in place forever. `learned_conclusion.superseded_by`
finds earlier conclusions to the SAME question; a different conclusion is written
with them hidden from the dedup snapshot and they go to the archive, not away;
the same conclusion writes nothing. Witness:
`test_a_corrected_conclusion_replaces_the_old_one`. The library experiment gained
a recheck phase: two deliberately wrong conclusions are planted in the memory
arm, then all twelve questions are asked with the files open.

**Addendum: a correction is not an echo.** Fifth library run, recheck phase with
two planted wrong conclusions: both answers were corrected at the source
(12/12 right, 12/12 went to the source), but one wrong record stayed — the
corrected conclusion was refused as `memory_echo_suspected` (similarity 0.85 to
a recent agent-auto write). A correction differs from the error by one word or
number, so the echo gate would silence every correction the dedup gate let
through. `remember(..., supersedes=True)` skips the echo gate for a write that
replaces an earlier conclusion to the same question (which goes to the archive,
so no echo chamber can form). Witness:
`test_a_correction_is_not_silenced_as_an_echo` (without the exemption: reject,
similarity 0.89).

## The first work exam (2026-09-19, exam/work_exam.py)

Ten client-shaped tasks (calibration, orders with acceptance scripts, job choice,
work prepared for a human). First run 6/10 after fixing one checker of mine. The
four misses were agent defects; three fixed here, one left for a decision.

**The lab printed Cyrillic as «���».** On Windows the experiment wrote stdout in
the console code page while the tool decoded UTF-8; sales.csv's product names
arrived garbled, and the agent «restored» them by inventing «Кран, Смеситель,
Труба» — a fabricated deliverable. The probe now runs `-X utf8` (PYTHONIOENCODING
would be ignored under `-I`). Witness: `test_cyrillic_output_arrives_intact`.

**Running the tests was taken for the work.** «Read the code → run the tests»
counted as an effect batch with no failed step (red tests do not fail the
run_tests step), the observation round was skipped, and the agent stopped at a
correct diagnosis without writing the fix. `_effect_completed_cleanly` no longer
counts run_tests as the work, and red tests (`failed`/`errors`/non-zero exit)
always keep the round. The earlier comment promised exactly that and was false.
Witness: `test_running_the_tests_is_not_the_work_and_red_tests_keep_the_round`.

**A requested forecast was stripped as unsupported.** «Do nothing yet — forecast
whether you can do each task and how many steps» — the low-evidence gate removed
the whole answer (0 of 10 claims sourced). A forecast is the plan genre: its
vocabulary (прогноз, оценк, справлюсь, шагов, forecast, estimate) joins
`_PLAN_MARKERS`; the genre's guards (no result report, no external fact) stay.
Witness: `test_a_requested_forecast_is_not_stripped_as_unsupported`.

**Doctrine routes read documents that are not there.** The morning's verifier
fix covered one route; «would you take this job … without other people's
passwords or money» matched the self-repair route, the planner read
knowledge/doctrine/… five times in an order folder, the attempt budget ran out,
and «What exactly should be built?» was glued onto a finished answer. The
doctrine, subagent, memory and self-repair routes (injection and prompt
directive) now fire only where one of their documents exists
(`doc_routing.doctrine_docs_present`). Eleven planner tests that pinned the
routes now build a repo-like workspace. Witness:
`test_a_doctrine_route_is_silent_where_its_documents_are_absent`.

Left for a decision: an order with an unspecified format and recipient («make
the client a report in the format he needs and send it») was answered with a
Markdown default, an honest «cannot send», and the unknowns listed — not a
clarifying question.

**Addendum: second work run — ask first, see your tools, fill the template,
run the code.** Second run: 5/10, and every miss had a cause:
(1) The operator decided that an order whose choices are left to an unnamed
person («в нужном ему формате», «отправь ему» without an address) is ASKED, not
guessed. `completion_contract._deferred_choices` turns exactly that into an
ambiguity phrased as the question to the client; the existing sixth gate
(`_contract_ambiguity_gate`) asks it before any work. Measured on 90 real
requests (75 exam, 15 live from the agent's logs): fires on the one it should.
Witness: `tests/test_a_choice_left_to_someone_else_is_asked.py`.
(2) «Can you send this by e-mail?» — «yes»; «would you mail 200 ads?» — «take».
The synthesizer never saw its tool list. `<runtime_self>` now carries `tools`
(registered minus policy-blocked), citable as [runtime:tools].
(3) A reference sheet shipped with every citation as «(стр. N)»: the request
showed the format and the agent copied the sample. `unfilled_placeholders`
(«стр. N», «N шагов», ГГГГ-ММ, <значение>) keeps the observation round open and
names the hole in the round's block.
(4) A delivered function had a stray Boltzmann constant — off by 10^23 — and was
never run once. The planner prompt: code handed over is run once, with a
checkable input, before it is handed over.
Witnesses: `test_an_unfilled_template_is_not_finished_work`,
`test_the_agent_sees_its_own_tools`.

**Addendum: third work run — a stale cache undid a correct fix; the client's
format and the agent's decisions reach the answer.** Third run 6/10; the order
with a missing format now ASKS (pass), and the formula is right (pass).
(1) «Fix it by the client's tests»: attempt 2 wrote the correct fix and planned
the tests again; the working-memory cache served the RED run taken before the
fix, and attempt 3 reverted the correct code. The cache served any
(tool, arguments) match, forever — the same mechanism showed old numbers.txt
after the client appended lines between turns (the morning's exam, «served from
working-memory cache»). `core/cache_freshness.py`: a stored result is served
instead of a call only if its world cannot have changed — web reads within the
session, file_read while the file's mtime and size are the same; tests, the
lab, workspace search/listing, writes, time and memory are always called again.
Storage is unchanged (results still serve as later turns' evidence). Witness:
`tests/test_a_changed_world_is_not_served_from_cache.py`; the citation-refusal
test now keeps its sources on disk (a cache that served a file that does not
exist was the defect).
(2) «Answer strictly in lines ПРОГНОЗ n: …» — the answer described the format
(«three forecast lines») instead of writing it; the output contract now says a
prescribed format goes into Conclusion verbatim, filled in.
(3) «N: БЕРУ / НЕ БЕРУ» over fifteen orders was erased whole by the
low-evidence gate; a decision about one's own work joins the plan genre.

**Addendum: a tool's name is not a fact about the world.** Fifth work run 9/10;
the miss: an estimate («4 steps … the count is done by python_probe») was
erased by the low-evidence gate — `_states_external_fact` matched «python»
inside the agent's own tool name as the external entity Python, and the plan
genre lost its exemption. Entities now match as whole words only
(`_ENTITY_RE`); «Python 3.13 вышел …» still needs a source. Witness:
`test_a_tool_name_is_not_a_fact_about_the_world`.

**Addendum: a restated figure and a self-fact are not world claims; the internet
is not memory.** Sixth work run 9/10; the triage of fifteen orders was erased
again: it restated the order «a $500 trade» (the $ marks an external fact) and
said «Python 3.11.9 is installed [runtime:python_version]». `_states_external_fact`
now takes the user's question as `known`: a number-with-unit expression that the
question already contains is a restatement («$500» — yes, «$900» — a new claim),
and a line cited [runtime:…] is measured about the agent itself. Both gate call
sites pass `question`. Witness:
`test_a_restated_user_figure_and_a_self_fact_are_not_world_claims`.

Two worlds, two statuses (operator, 2026-09-19): the local library is the source
of truth; the internet is a source still to be qualified. A conclusion resting
on web sources is not written to long-term memory by `learned_conclusion`; it
stays experience in the episode. Witness:
`test_a_conclusion_from_the_internet_is_not_made_a_fact`. The web exam
(exam/web_exam.py) measures the rest, including web records in long-term memory.

**Addendum: a flagged page is still evidence; documentation is not an attack;
stuck on its own draft, the agent does not ask the human.** First web exam
(2026-09-19): 7/9 by the checker, 5/9 after the checker learned the second form
of «asking the human». Four tasks (rich version, fastapi minimum Python, RFC for
HTTP/2, PEP for f-strings) failed the same way:
(1) The injection guard flagged ordinary documentation `suspicious` — the
override pattern matched a bare word followed by whitespace («Copy PIP
instructions», «the following command», `tasks = …`), the role pattern matched
the description «Intermediaries act as both client and server». 5 pages of 9.
Override now needs a claim of replacement (new/updated/system … instructions,
or an «Instructions:» heading); «act as» only as an imperative (sentence start,
after «please / you must»). The old bare-word pattern is kept verbatim in
test_injection_scan_scope.py as the witness of the root cause it once caused.
(2) The `suspicious` annotation turned the `web_fetch` dict into a string; the
evidence factory builds `web_page` only from the dict, so the fetched page never
entered the chain. The answer cited it, the verifier counted 5 «fabricated»
citations, the replans ran out. `annotate_suspicious_output` now wraps only the
untrusted fields (`web_fetch.text`, `shell_exec` stdout/stderr, `run_tests`
tails) and keeps the envelope; MIR-011's quarantine (flagged sentences become
`suspect` claims) finally sees web pages. Witness:
`test_a_flagged_page_stays_evidence`.
(3) With replans exhausted, the clarification gate prepended «Уточни: что
именно нужно построить? где? что считать готовым?» to fully specified lookup
questions. When every failure behind the exhaustion lies in the agent's own
draft (`unresolved_citation`, `claim_refuted`), the frame questions are skipped
and `clarification_gate_skipped` is journalled; an unknown or external cause
still asks. Witness: `test_a_stuck_draft_does_not_ask_the_human`.

**Addendum: second web exam — the name Dan, a lost TLS context, the agent's own
journal, a browser challenge.** Run 2 (fresh draws; run without the
observation-round flag, so one attempt per task): 5/9, 0 web records in memory.
(1) RFC 3986 was BLOCKED whole: `\b(DAN|STAN|AIM)\b` with IGNORECASE matched
«Dan Connolly», «Dan Kohn» in the acknowledgements. The persona acronyms now
need jailbreak context («DAN mode», «you are DAN», «DAN (…»); case stays
insensitive — «Enable dAn mode» is still blocked. After this and the first
addendum, all 40 web results of both runs scan clean.
(2) `peps.python.org` failed with CERTIFICATE_VERIFY_FAILED (Windows store
lacks the intermediate). The guarded `https_open` called `do_open` without
`context=`, so no context given to the handler ever reached the connection.
It now passes `self._context`, built on certifi's roots when installed (as
requests does; mixing both stores still fails — measured). Expired, wrong-host
and self-signed certificates are still refused (checked live). Witness:
`test_the_tls_context_reaches_the_connection`.
(3) `find_in_files` searched `logs/` and `data/` — the agent's own journal and
memory — and for «release» found only this turn's trace, which contains the
query itself; the answer was «not found» over the echo of its own question.
Top-level `logs/` and `data/` are skipped unless `path` points inside them.
Witness: `test_the_agents_own_journal_is_not_searched_as_the_workspace`.
(4) PyPI answered a missing package with 200 and a «Client Challenge» body; the
error-page rule counted wrapped lines as six sentences and let it through.
Browser-challenge markers now identify a short body on their own. Witness:
`test_a_browser_challenge_is_not_the_page`.

**Addendum: a clear request with gathered evidence is not a frame problem.**
Third web run (observation round on, fresh draws): 8/9, 0 web records in
long-term memory; the TLS, DAN, journal and challenge fixes all held (RFC 9110,
PEP 498 opened on peps.python.org, the missing package reported absent). The
miss, N04 («minimum Python for ruff»): two observation rounds spent the budget,
on the third the lab rightly refused network access (`tool_error`), and the
answer «3.7» — PyPI page opened, seven claims verified — began with «what
exactly should be built?». `frame_questions_help` now also returns False when
the completion contract saw no ambiguity and the run gathered artifacts; a run
that gathered nothing (failing tool, unparsable plan) still asks, as the pinned
tests require. Witness:
`test_a_clear_request_with_gathered_evidence_is_not_a_frame_problem`.

**Addendum: a JSON API answer keeps its facts in view.** Same N04, root cause:
https://pypi.org/pypi/ruff/json is 1 MB (cut at the fetch limit); the field
`info.requires_python` sat at 20 KB behind the README in `info.description`,
while an observation shows the agent about 7 KB. It then tried to read the
field through the lab over the network and was refused. `tools/json_view.py`
renders a JSON body compactly — strings over 300 chars, lists over 20 items and
objects over 50 keys clipped, each clip stated; a body cut at the size limit is
parsed member by member and only complete top-level members are shown. Live:
the ruff answer is 2.9 KB with `requires_python` at 2.4 KB; the GitHub
release's `tag_name` at 1.8 KB. `content_hash` still covers the full body.
Witness: `test_a_json_answer_keeps_its_facts_in_view`.

**Addendum: the knowledge pipeline was the second door from the internet into
memory.** Fourth web run: 9/9 — but 4 records in long-term memory came from the
web. The page https://www.rfc-editor.org/info/rfc6749/ was catalogued as
«documentation» (trust 0.75) and its fragments — «0 Authorization Framework
D.», «Internet Engineering Task Force (IETF) D.» — were auto-written as facts.
`learned_conclusion` already kept web conclusions out; `KnowledgePipeline.run`
now takes `admit_internet` and the turn passes False: a claim whose source is a
web page, search hit, feed or http(s) locator stays in the source registry (to
be qualified) with a `two_worlds` decision row. Deliberate ingestion
(`core/ingestion.py`) keeps the default and still admits it. Witness:
`test_the_internet_is_not_made_memory_by_a_turn` (the wiring test drives
`_catalogue_chain` on a stub host; it fails when the turn passes True).

**Addendum: new task types, and a comparison with a missing side.** A second
web task set (exam/web_exam.py --set 2: licence, GitHub release date, which of
two packages is newer, a world fact stale in our own doc, a dependency yes/no,
Node.js LTS, live star count) — shapes the agent had never seen: 6/7 on the
first run, 0 web records in memory. The miss, «pydantic or sqlalchemy — which
latest release is later?»: the agent found pydantic's date (2026-08-28), did
NOT find SQLAlchemy's (2026-09-15; its JSON is >1 MB, cut inside `releases`, so
`urls` with the upload times was lost; on the HTML page «Released: Sep 15,
2026» sat at 6.3 KB behind lines holding a single space) — and still answered
«pydantic». Two changes: the output contract says a comparison needs every
