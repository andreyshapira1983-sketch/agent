...
| падения в облаке | все 10 из-за окружения: OCR/песочница 5, office 1, https-сторож 2, нет `fastapi` 2 |
...
| `core/patch_route.py` | 90,8% | в `settle_patch`: **правка трогает запретное (286–287)**, **правка ослабляет существующих судей (306–307)**, путь применения не дошёл до коммита (323–324), сбой слияния (327–328), файла правки нет (279) |
...
1. **`patch_route.settle_patch`**: правка, трогающая запретное (`_FORBIDDEN`), обязана остановиться; правка, ослабляющая судей, тоже. Это тормоза самопочинки, и сейчас их ветки не исполняет ни один тест.
