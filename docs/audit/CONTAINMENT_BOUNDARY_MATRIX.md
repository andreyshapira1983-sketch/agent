...
| `memory` | capability | **yes** | yes | core/loop_step_execution.py:417 (`decision = self.policy.check(action)`) and its deny at :418; for effectful t… |
...
| `approval_provider` | permission envelope | **yes** | yes | core/loop_step_execution.py:893 — `if self.approval_provider is None:` emits `code="approval_unavailable"` and… |
...
| `verifier_enabled` | permission envelope | **yes** | yes | Two, in opposite directions. (1) Output: core/unsupported_claims.py:418 `if evidence_expected and fabricated >… |
