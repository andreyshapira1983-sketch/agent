...
| H-41 | 2004- | слабый или усечённый хеш как удостоверение | коллизии MD5 в решениях о доверии | документ, подписанный для одного, принимается за другой, потому что удостоверением служит хеш | каждое место `hashlib` и то, ЧТО решают по отпечатку | (а) сплошной поиск слабых алгоритмов; (б) арифметика столкновений на живых объёмах; (в) проследить, к какому решению ведёт самый ответственный отпечаток | (а) **md5/sha1 — ноль**; (б) **48 бит при 137 заявках ≈ 3e-11, 64 бита ≈ 6e-16** — класс не достаёт; (в) **отпечаток ведёт к разрешению на НЕОБРАТИМЫЕ эффекты, и истёкшее разрешение продолжало разрешать** | NOT APPLICABLE по хешу → FIXED по тому, куда он ведёт | проба про хеш нашла дефект не в хеше |
| H-42 | 2000s | синхронный залп: отправители сходятся в одну точку разом | TCP incast; семейство «повторы без разброса» | одна причина роняет N работ, все получают одинаковую отсрочку и возвращаются в одну секунду, снова вместе падая | `_RETRY_BACKOFF_*` и `_failure_transition` в очереди задач | (а) есть ли разброс в отсрочке; (б) уронить шесть повторяемых задач одной причиной и прочесть моменты готовности; (в) кто вообще заводит повторяемую задачу | (а) **разброса нет нигде**; (б) **все шесть готовы в одну и ту же секунду — залп воспроизведён**; (в) **ни одно место в коде не ставит `max_attempts>1`, и все 21 живая строка несут 1** | механизм воспроизведён, достижимость НОЛЬ на живом пути | и сама экспоненциальная отсрочка вместе с ним недостижима |
| H-43 | 2000s | значение становится СТРУКТУРОЙ для следующего разборщика | SQL-инъекция как расхождение разборщиков | данные проскакивают в слой, который читает их как разметку, и меняют смысл записи | построчный JSONL состояния и пометки проверки в тексте ответа | (а) вписать перевод строки и целый конверт внутрь значения; (б) написать `[verified:web:1]` в ответ и посмотреть, купит ли это проверку | (а) **запись не расщепляется — `json.dumps` экранирует**; (б) **пометки никто не ЧИТАЕТ как доказательство: их пишет верификатор и снимает показ** | NOT APPLICABLE | побочно: словарь чистки покрывает 3 из 4, и четвёртая — предупреждение |
| H-44 | 2007 | проход-починка приводит значение к «правильному», и оно оказывается другим по смыслу | Dynamo: согласованность в конечном счёте и восстановление чтением | реплики расходятся, примирение выбирает победителя по правилу, а правило не знает, что значат значения | `status` процедурной памяти: рождение, формула и `recompute_legacy_confidence` | сверить ХРАНИМОЕ производное с тем, что даёт формула, на живых 34 процедурах | **24 из 34 расходятся: записано `candidate`, формула даёт `needs_review`; у поля оказалось ТРИ власти, и `needs_review` исключается из выдачи** | **LOCALLY REPRODUCED → FIXED: одна власть, и незнание отделено от приговора** | прогон починки отключил бы 71 % процедурной памяти |
| H-45 | 2013- | двое считают себя единственным | Paxos/Raft: расщепление мозга | два узла одновременно верят, что держат аренду, и оба действуют | `SingleInstanceLock`, захват задачи, путь замка | (а) взять замок дважды разными написаниями одного пути; (б) захватить задачу по УСТАРЕВШЕМУ взгляду после чужого завершения; (в) взять замок после освобождения | (а) **отказано: написания сводятся к одному файлу**; (б) **отказано `TaskAlreadyClaimed` — состояние перечитывается под замком**; (в) **берётся заново — мёртвый процесс не блокирует навсегда** | ALREADY PROTECTED, три оси | замок опирается на advisory-замок ОС, а не на PID-файл |
| H-46 | 2012 | граница календаря: секунда повторяется, дня не существует | живая блокировка ядра на високосной секунде; сертификат Azure 29 февраля | арифметика по компонентам даёт несуществующую дату, а повторившаяся секунда рушит допущение о строгом возрастании | всякая арифметика дат и всякий выбор «свежайшего» | (а) сплошной поиск `.replace(year/month/day)`, `relativedelta`, «год = 365 дней»; (б) сколько ПОВТОРОВ отметок живьём; (в) кто выбирает свежайшую запись и по чему | (а) **ноль во всём дереве, детектор доказан на `replace(year=…)`**; (б) **1053 повтора из 10382 — 10 %, а в профиле 253 из 254**; (в) **по ПОЗИЦИИ в файле, не по времени** | NOT APPLICABLE по календарю; повторы часты и безвредны по построению | предметность повторов измерена, а не отброшена |
| H-47 | 2012 | автоматическое переключение само причиняет вред | отказ переключения GitHub MySQL | проверка здоровья запускает переключение, переключение ломает больше, чем чинило, и автоматика начинает мигать | переход между провайдерами и понижение здоровья (MIR-132) | (а) чем ограничен цикл переключения; (б) понижает ли ВРЕМЕННЫЙ сбой здорового провайдера | (а) **числом провайдеров: каждый пробуется однажды**; (б) **три квотных маркера числились временными и не могли сработать НИКОГДА — их поглощала голая подстрока `quota` в стойком списке** | поведение верно, слова были ложны → мёртвые маркеры убраны, граница закреплена | сужать стойкий список нельзя: это вернуло бы MIR-132 |
| H-48 | 2015 | устаревший путь, который все считали неиспользуемым | GHOST (CVE-2015-0235) в `gethostbyname` | дыра жила в функции, объявленной устаревшей; никто не думал, что её ещё зовут, и потому никто не проверял её границы | слой совместимости `core/subagent_contract.py` (440 строк, два устаревших контракта), достижимый из СОХРАНЁННЫХ заявок | (а) есть ли живые заявки старой формы; (б) одинаково ли строги старый и новый пути | (а) **ноль: из 137 заявок ни одной про подагентов**; (б) **проверка живёт в `__post_init__` самого договора, а не в разборе, поэтому оба пути идут через неё** | ALREADY PROTECTED по построению | старый путь не может быть мягче, если строгость не в разборщике |
| H-49 | 2022 | правка сносит и то, чем её чинят | маршрутизация Rogers | изменение ядра сети убрало сеть ВМЕСТЕ с каналом управления: инженеры не могли достучаться до оборудования, потому что управление шло по той же сети | лента самоприменения против тика, запуска и строки состояния | (а) применяет ли код сам ТИК; (б) карта досягаемости ленты; (в) закреплена ли граница | (а) **нет: тик только считает и докладывает, применяет человек отдельной командой**; (б) **тик, `main.py`, `app/`, `scripts/`, `.github/`, `config/` — вне досягаемости**; (в) **не была закреплена ничем, а список границ лежит в файле, который лента правит** | разделение верно → граница получила свидетеля | расширение полномочий было видно лишь строкой в диффе |
| H-50 | 2005- | запись вернула успех и не дошла до диска | согласованность файловой системы при обрыве: ext3 `ordered` против `writeback`, семантика `fsync` | успешный возврат означает попадание в кэш, а не на носитель; при потере питания хвост теряется или рвётся | `append_state_jsonl_unlocked` против `_atomic_write_lines` | подсмотреть сам системный вызов на пути дописывания и измерить цену барьера | **барьер стоял только на атомарной перезаписи; на ДОПИСЫВАНИИ — горячем пути каждого резерва бюджета, строки расхода и обновления задачи — его не было. Цена: +0,628 мс на строку, около 2 с за месяц на живом объёме** | **LOCALLY REPRODUCED → FIXED** | потеря хвеста бьёт в ту же сторону, что H-27: потолок забывает потраченное |
| H-51 | 2022 | восстановление рассчитано на одну единицу, а понадобилось для многих | многодневный отказ Atlassian | скрипт снёс не те цели (закрыто H-36), а простой растянулся на НЕДЕЛИ потому, что путь восстановления существовал для одного объекта и не существовал для многих | `scripts/restore_drill.py` и охват копиями | перебрать ЖИВЫЕ хранилища и спросить, у какого из них есть копия | **учение перебирало КОПИИ и показывало девять зелёных строк, тогда как 19 живых хранилищ из 24 не имели ни одной копии — включая журнал бюджета, ящик одобрений и расход моделей; `data/` исключён из git** | **LOCALLY REPRODUCED → прибор исправлен, охват назван; политика копий вынесена оператору** | ошибка была в приборе: он перебирал не то множество |
...
### 2000-е
- [x] Y2K-подобные переполнения эпохи → H-25 (временная ось; метка не была поставлена — исправлено 2026-08-25)
- [x] TCP incast → H-42 (залп воспроизведён; достижимость ноль — некому повторяться)
- [x] византийский разброс часов → H-40 (местная форма: часы ДВИГАЮТСЯ)
- [x] коллизии MD5 в решениях о доверии → H-41 (хеш крепок; дефект нашёлся там, КУДА он ведёт)
- [x] SQL-инъекция как расхождение разборщиков → H-43 (не достаёт; побочно защищена граница чистки)
- [x] Therac-класс: обход блокировки в планировщике → H-21

### 2004–2010
- [x] согласованность файловой системы при обрыве (ext3, семантика fsync) → H-50 (барьер на горячем пути дописывания)
- [x] сквозные контрольные суммы ZFS против тихой порчи → H-29
- [x] Dynamo: согласованность в конечном счёте → H-44 (три власти над полем; починка отключила бы 71 % памяти)
- [x] Paxos/Raft: расщепление мозга → H-45 (три оси; владение не выводится, а держится ОС)
- [x] лавина повторов memcached → H-28
- [x] энтропия Debian OpenSSL CVE-2008-0166 → H-30

### 2011–2016
- [x] високосная секунда → H-46 (повторы отметок живьём 10 %, но «свежайшее» решает позиция)
- [x] шторм перезеркаливания AWS EBS (2011) → H-32
- [x] отказ переключения GitHub MySQL → H-47 (три маркера, которые не могли сработать)
- [x] рассинхрон развёртывания Knight Capital (2012) → H-31
- [x] Heartbleed (2014) → H-34
- [x] Shellshock (2014) → H-35
- [x] GHOST → H-48 (слой совместимости достижим, но строгость стоит на объекте)
- [x] сертификат Azure 29 февраля → H-46 (арифметики по компонентам нет ни одной)
...
### 2022–2026
- [x] многодневный отказ Atlassian → H-51 (третья часть класса: 19 хранилищ из 24 без копий)
- [x] маршрутизация Rogers → H-49 (планы разделены; граница получила свидетеля)
...
**177** коммитов, ни один из которых не отправлен на удалённый репозиторий.

**Поправка того же дня.** Сначала здесь и в сообщении коммита стояло «190» —
число, названное по памяти, а не прочитанное. Настоящее значение
(`git rev-list --count origin/main..HEAD`) — 177. Вывод от этого не меняется,
но само число было улуликой в постоянной записи, и правило «идентификатор
читают, а не вспоминают» нарушено мной в тот же день, когда я применял его к
чужому коду.
...
### H-41 — проба про хеш нашла дефект не в хеше

Коллизионный вопрос закрылся арифметикой, а не мнением: слабых алгоритмов нет
ни одного, все отпечатки sha256, и при живых объёмах (137 заявок, 142 эпизода)
вероятность столкновения усечённого до 48 бит — около 3e-11, до 64 бит — 6e-16.
Класс до нас не достаёт.

Но проба спрашивала не «крепок ли хеш», а «к какому РЕШЕНИЮ он ведёт», и самый
ответственный оказался вот каким: разрешение на НЕОБРАТИМЫЕ эффекты ищется по
ключу `allow_effects:<16 знаков от цели>` среди `list(status="approved")`.
Этот читатель истёкшие не отсекает — `expire_stale` трогает только `pending`.
Замер: заявка со сроком, прошедшим тридцать дней назад, невидима для
`pending()` и полностью видима здесь. То есть просроченное разрешение
продолжало разрешать необратимое.

**Форма знакомая — расхождение двух читателей одной записи (H-26), приложенное к
полномочию.** Один читатель считает заявку мёртвой, другой живой, и путь
разрешения брал того, кто мягче.

**Смысл поля не выдуман, и это была развилка.** Сперва я решил, что `expires_at`
— дедлайн ЗАЯВКИ («ответь за N часов»), и что фильтровать по нему одобренное
значит читать поле не в том смысле; на этом я едва не остановился. Проверка
соседа опровергла: `_active_standing_grant` уже читает `expires_at` у ОДОБРЕННОЙ
заявки как срок действия гранта и пропускает истёкшие, а сверх того держит
суточный потолок с учётом израсходованного. Два механизма разрешения на одно и
то же расходились ровно на защите.

**Что приведено и что нет.** Приведён срок — в той половине, где смысл поля уже
установлен этим же кодом. Потолок НЕ введён: сколько раз одно разрешение может
сработать, это решение о необратимом и принадлежит оператору, а не экспертному
дефолту. Отсутствие срока по-прежнему значит «бессрочно» — менять это тоже
значило бы вводить политику.

**Нечитаемая отметка — отказ, а не разрешение.** Направление выбрано по цене:
пропустить необратимое действие по непрочитанному сроку хуже, чем попросить
новое одобрение.

### H-42 — залп собирается, но собираться не из чего

Разброса в отсрочке нет: `base * 2^(attempts-1)` с потолком в час, ни единого
`jitter` во всём дереве. Проба подтвердила следствие буквально — шесть задач,
упавших от ОДНОЙ причины, получили один и тот же момент готовности,
`12:00:30`, секунда в секунду. Если причина держится, они вернутся вместе,
упадут вместе и снова разойдутся строем.

**Достижимость — ноль, и это доказано двумя независимыми способами.** Ни одно
место в `core`, `app`, `cli`, `agent_tick` не создаёт задачу с
`max_attempts > 1`; и все 21 живая строка очереди несут `max_attempts = 1`.
Задача становится терминальной на первом же отказе, поэтому строю не из кого
собраться.

**Побочно и честно: вместе с залпом недостижима и сама отсрочка.** Ветка
экспоненциального повтора (`_RETRY_BACKOFF_BASE_SECONDS`, удвоение, потолок
часа), заведённая ради OFM-010 / CORE-07, срабатывает только при
`max_attempts > 1`. На производственном пути её не достигает никто. Это не
дефект и не повод удалять — механизм законный и ждёт своего производителя, — но
записать это надо, потому что «есть отсрочка» и «отсрочка работает» разные
факты.

**Условие, при котором запись перестаёт быть верной**, названо: первый же
производитель, ставящий `max_attempts > 1`, включает разом и отсрочку, и залп.
Тогда разброс перестаёт быть роскошью. Сегодня строить его значило бы платить
за опасность, которой нет, и вдобавок завести первый в дереве вызов
`random.*` — который тут же поймает сплошная проба слабых источников
случайности из H-30.

**Моя проба была неверной с первого раза, и это поймано до вывода.** Первый
заход создавал задачи с настройкой по умолчанию, они уходили в терминальный
отказ, а `run_after` оставался от создания — я мерил поле, которого гипотеза не
касалась, и получил «разнесены» там, где ничего не проверялось. Разброс в восемь
миллисекунд был временем цикла, а не защитой.

### H-43 — класс не достаёт, а побочная находка защитила код от «дополнения»

SQL у нас нет, но форма переносится: значение попадает в структурный контекст и
меняет структуру. Проверены оба наших контекста.

**Построчный JSONL.** Перевод строки внутри значения записи не расщепляет:
`json.dumps` его экранирует, читается одна строка. Полезная нагрузка, несущая
собственное поле `_integrity`, сохраняется как ДАННЫЕ и конверт не подменяет —
проверка считается по внешнему полю.

**Пометки проверки в тексте ответа.** Здесь класс мог бы достать: если бы кто-то
читал `[verified:web:1]` как свидетельство проверки, модель написала бы его
сама. Не читает никто — сплошной поиск читателей пуст. Пометки пишет
верификатор, а показ их снимает; подделать проверку нечем.

**Побочная находка, и она развернулась против первого впечатления.**
`_strip_verification_markers` снимает `[unverified]`, `[verified:…]`,
`[declared:…]` и НЕ снимает `[topic-only:…]`, при том что докстринг обещал
снимать «пометки проверки» вообще. Читалось как неполный словарь: метка
появилась 26 июля, чистка написана 4 августа, автор её видел.

Живой замер перевернул вывод. `topic-only` — самая частая пометка (80 вхождений
против 46 у `unverified`), и 27 из них стоят прямо в `full_answer`, то есть
человек их видит. А значит она и есть **предупреждение**: источник по теме, но
утверждение им не подтверждается. Дописать её в чистку значило бы сделать ответ
увереннее, чем он есть, — починить надпись ценой правды.

Поэтому исправлен ДОКСТРИНГ, а не код: полна была как раз реализация. Граница
закреплена тестом, который краснеет при добавлении `topic-only` в выражение
чистки, и отдельной растяжкой на объяснение — потому что именно молчание
границы заставило прочесть полный код как неполный.

### H-44 — три власти над одним полем, и починка была бы хуже болезни

Сверка хранимого производного с формулой на живых данных: `confidence` сходится
у всех 34 процедур, а `status` расходится у 24. Записано `candidate`, формула
для тех же чисел даёт `needs_review`.

**Обвинение рассыпалось наполовину и стало точнее.** Все 24 — новорождённые:
`success_count=0`, `failure_count=0`, `confidence=0.5`. Это не порча данных, а
разные ПРАВИЛА. У поля их оказалось три: рождение ставит `candidate`,
`_procedure_status_for` для тех же чисел даёт `needs_review`, а
`recompute_legacy_confidence` знала только `active`/`needs_review` и `candidate`
не производила вовсе.

**Цена измерена, а не предположена.** `needs_review` исключается из выдачи
(`if proc.status in {"obsolete", "needs_review"}`), `candidate` используется.
Значит проход-починки тихо отключил бы 24 процедуры из 34 — 71 % процедурной
памяти, — при том что ни один факт о них не изменился. Это ровно форма Dynamo:
примирение приводит реплику к «правильному» значению, не зная, что значения
значат.

**Неверным оказалось не рождение, а формула.** Она смотрела только на
уверенность и потому сваливала «ещё не проверено» и «проверено и не годится» в
один исход. У ни разу не запускавшейся процедуры уверенность — это ПРИОР (0.5),
а не результат; `needs_review` для неё есть приговор без суда.

Починка: обе половины сразу. Формула получила `failure_count` и различает
полное отсутствие опыта; починка перестала изобретать свой статус и зовёт ту же
формулу. Властей над полем снова одна. Живые данные не трогались — 24 строки
уже несут `candidate`, и теперь любой проход это подтвердит, а не переименует.

**Границы закреплены с обеих сторон:** послабление действует только при обоих
нулевых счётчиках, а процедура, которая действительно провалилась, по-прежнему
уходит в `needs_review`. Без второго теста первый чинился бы отключением
приговора вообще.

### H-45 — защищено выбором механизма, а не бдительностью

Расщепление мозга опасно там, где владение арендой ВЫВОДИТСЯ: по PID-файлу, по
отметке времени, по «я же брал». Здесь оно не выводится — владение держит
advisory-замок операционной системы, и это решает три вопроса сразу.

**Мёртвый процесс.** Замок отпускает ОС, а не запись в файле, поэтому
осиротевший файл не блокирует старт навсегда. Проверено: после освобождения
замок берётся заново.

**Два написания одного пути.** `data/daemon.lock`, `./data/daemon.lock`,
`data/../data/daemon.lock` — второй захват отвергнут во всех случаях, потому что
это один файл, а не одно имя. Проба доказана контролем: на ДРУГОМ пути замок
берётся, то есть она умеет показывать положительное, а не отказывает всегда.

**Устаревший взгляд.** Второй экземпляр хранилища, видевший задачу как
`pending` до чужого завершения, получает `TaskAlreadyClaimed`: `mark_running`
перечитывает состояние под файловым замком, а не верит своей памяти. Это и есть
та половина класса, где узел действует по копии, которой больше нет.

**Остаток, названный честно.** `held` — кэшированный булев признак («я открыл и
запер»), и потребители вроде `recover_orphaned_tasks` спрашивают его, а не ОС.
Пока владение живёт в дескрипторе объекта, признак верен; расходиться ему не с
чем. Но это утверждение о механизме, а не измерение: сценария, где дескриптор
жив, а замок утрачен, я не построил и потому ничего о нём не заявляю.

### H-46 — два класса одной границы, и один из них оказался предметным

**Сертификат Azure 29 февраля.** Беда там родилась из арифметики по КОМПОНЕНТАМ:
«сегодня плюс год» на 29 февраля даёт дату, которой нет. У нас такой арифметики
нет ни одной: ноль `.replace(year=/month=/day=)`, ноль `relativedelta`, ноль
выражений вида «год = 365 дней». Всё время считается `timedelta` в секундах,
минутах, часах и днях, а такая арифметика несуществующей даты не производит.
Детектор доказан прежде, чем ему поверили: он ловит `d.replace(year=d.year+1)` и
правильно пропускает `d.replace(tzinfo=utc)`.

**Високосная секунда.** Здесь я ожидал такого же пустого результата и ошибся бы,
если бы не померил. Повторяющиеся отметки времени у нас НЕ редкость: 1053 из
10382 живых отметок совпадают с соседями — 10 %, — а в `user_profile.jsonl`
совпадают 253 из 254. Допущение «время уникально» было бы ложным.

Вреда, однако, нет, и причина конкретна: «свежайшая запись» нигде не выбирается
по времени. `UserProfile.load` идёт по файлу НАЗАД и берёт первую разобравшуюся
— то есть решает позиция записи, а не отметка. Сортировки, где порядок влияет на
удержание, либо несут явный доразрыв (`(created_at, id)`), либо опираются на
устойчивость сортировки Python, которая при равных ключах сохраняет входной
порядок, а входной порядок — это порядок дописывания.

**Что было бы дефектом.** Первый же читатель, который начнёт выбирать свежайшее
через `max(created_at)`, получит произвольный результат на 10 % данных. Сплошной
поиск таких читателей сегодня пуст, и это условие названо, чтобы его заметили.

### H-47 — три маркера, которые три недели описывали поведение, которого нет

Цикл переключения ограничен: `_next_credentialed_provider` возвращает
провайдера, ещё не пробованного в этом вызове, поэтому один логический вызов
стоит не больше одной попытки на провайдера. Учёт при этом честен — живой замер
H-27 показал, что резервы и строки расхода сходятся по каждому провайдеру.

Вторая половина класса — «автоматика паркует здорового» — и вскрыла ложь.
В списке ВРЕМЕННЫХ маркеров стояли `insufficient_quota`, `insufficient quota` и
`exceeded your current quota`, а в списке СТОЙКИХ — голая подстрока `quota`.
Стойкий проверяется и поглощает все три: сработать они не могли никогда, при
том что комментарий рядом утверждал, будто временный класс «переключает, но не
понижает».

**Чинить надо было не код.** `insufficient_quota` у OpenAI означает исчерпанный
СЧЁТ, а не минутный лимит; понижать за него правильно. Сузить стойкий список
значило бы вернуть класс MIR-132 — один пустой баланс, повторённый 391 раз за
четыре дня, потому что ничто не помнило отказ между процессами.

**Живьём поглощение не навредило**: 391 ошибка провайдеров про баланс и НИ ОДНОЙ
со словом «quota». Вредил только текст.

Поэтому мёртвые маркеры убраны, а причина оставлена на их месте, и граница
закреплена с обеих сторон: исчерпанный счёт обязан понижать, ограничение
частоты — обязано НЕ понижать. Отдельная растяжка краснеет, если во временном
списке снова заведётся маркер, поглощаемый стойким: маркер, который не может
сработать, есть ложное обещание, и один раз оно уже прожило три недели.

### H-48 — устаревший путь безопасен не по внимательности, а по месту проверки

GHOST жил в `gethostbyname` — функции, которую считали неиспользуемой, и потому
её границы никто не пересматривал. Местная форма у нас есть: `subagent_contract`
несёт 440 строк адаптеров к двум устаревшим моделям договора, и один из них
читает СОХРАНЁННЫЕ заявки одобрения, то есть достижим из состояния на диске, а
не только из свежего кода.

**Живых входов ноль**: из 137 заявок в ящике ни одной про подагентов. Но урок
GHOST про достижимость, а не про трафик, поэтому вопрос ставился иначе: если
заявка старой формы придёт, получит ли она те же проверки.

**Получит, и по строению, а не по бдительности.** Оба пути — современный
`from_dict` и старый адаптер — заканчиваются одним объектом
`CanonicalSubagentContract`, а правило непустоты и проверки версии схемы и
уровня риска живут в его `__post_init__`. Строгость стоит НА ОБЪЕКТЕ, а не в
разборщике, поэтому старый конструктор физически не может оказаться мягче.

Проба доказана контролем: полный договор принимается (значит проба умеет
показывать положительное), а любое опустошённое обязательное поле —
`contract_id`, `name`, `role`, `objective` — отвергается независимо от того,
каким путём объект собирали.

**Что осталось непроверенным и названо.** Это ответ про ВАЛИДАЦИЮ, а не про
ценность слоя. Вопрос «нужны ли ещё эти 440 строк» остаётся открытым в аудите
балласта как B4, и здесь он не решается: доказано, что старый путь не опаснее
нового, а не то, что он нужен.

### H-49 — разделение планов оказалось верным, а его защита держалась на одном списке

Беда Rogers в том, что канал управления шёл по той же сети, которую снесли.
Местный вопрос ставится так же: может ли правка агента сломать то, чем эту
правку откатывают.

**Планы разделены, и это измерено.** Тик код НЕ применяет: он считает заявки и
печатает «столько-то готово к `:self-apply-run`», а применяет человек отдельной
командой на чистом дереве. Значит сломанный тик не блокирует починку, а лента не
зависит от тика.

**Карта досягаемости подтверждает разделение.** Вне досягаемости ленты:
`agent_tick.py` — то есть и сам тик, и строка `--status`, которой оператор видит
здоровье, — `main.py`, `app/bootstrap.py`, `app/single_instance.py`, `scripts/`,
`.github/`, `config/`. Наблюдение и запуск лежат вне рук наблюдаемого.

**А защита этого разделения не держалась ничем.** Границу задаёт один кортеж
`_ALLOWED_CODE_DIRS`, и лежит он в `core/self_apply_lane.py` — файле, который
лента править МОЖЕТ. Расширить собственные полномочия было бы обычной правкой,
заметной лишь строкой в диффе среди многих, и прошло бы те же ворота, что любая
другая.

Поэтому здесь не вводится новой власти и ничего не запрещается сверх
существующего: поставлен СВИДЕТЕЛЬ. Расширение границы теперь краснит тест с
названным следствием, а не проходит незаметно. Проверено ломкой: добавление
`scripts` в список немедленно краснит. Решение расширить по-прежнему за
оператором — но теперь оно требует переписать и объяснение, то есть быть
названным вслух.

**Что осталось в силе и не тронуто.** Лента правит саму себя — это ось MIR-139,
открытая решением оператора; тест закрепляет сам ФАКТ, а не запрет, чтобы его
изменение не прошло молча.

### H-50 — барьер стоял на редком пути и отсутствовал на горячем

Прошлая работа поставила барьер долговечности на `_atomic_write_lines`, и
докстринг там честно объясняет почему: `os.replace` атомарен для КАТАЛОГА, но
сам по себе не гарантирует, что данные легли на носитель, а пустой файл
состояния читается как законно пустое хранилище.

Ровно то же верно для ДОПИСЫВАНИЯ, и там барьера не было. Разница в частоте
работает против нас: перезапись случается при уборке, а дописывание — путь
каждого резерва бюджета, каждой строки расхода и каждого обновления задачи.
Потеря хвоста бьёт в ту же сторону, что H-27: потолок ЗАБЫВАЕТ потраченное, а
незаписанное обновление статуса открывает двойное исполнение.

**Цена измерена тем же способом, каким её мерили для перезаписи**, а не оценена
на глаз: +0,628 мс на строку; на живом объёме бюджета (3302 строки за месяц) это
около двух секунд суммарно — против тика, тратящего секунды на один вызов
модели.

**Проба смотрит на системный вызов, а не на текст функции.** Поиск слова `fsync`
в исходнике мерил бы адрес; здесь `os.fsync` подменяется и считается, сколько
раз его позвали. Границы закреплены отдельно: барьер ОДИН на вызов, а не на
строку (иначе измеренная цена перестала бы быть верной), пустой список работой
не считается, а отказ файловой системы оставляет запись такой, какой она была до
барьера — ровно как на пути перезаписи.

### H-51 — прибор перебирал не то множество, и его зелёный цвет значил меньше, чем выглядел

Класс Atlassian состоит из трёх частей, и «частично закрыт H-36» относилось
только к первой.

1. Разрушительная команда по неверным целям — **H-36**, закрыто.
2. Жёсткое удаление там, где есть мягкое — у нас мягкое: архив памяти,
   карантин строк, терминальный статус вместо стирания.
3. **Восстановление для одного, когда нужно для многих** — это и осталось.

Третья часть и стоила Atlassian недель: путь восстановления был рассчитан на
один объект, массового не существовало, и его пришлось изобретать под давлением.

**Наш замер вскрыл худшее.** `restore_drill.py` перебирал КОПИИ и потому не мог
задать вопрос «у чего копий нет». Он показывал девять зелёных строк. Перебор по
ХРАНИЛИЩАМ даёт другую картину: **19 живых из 24 не имеют ни одной копии** — в
том числе `budget_ledger` (деньги), `approval_inbox` (полномочия) и
`model_usage`. Проверено и опровергнуто очевидное возражение: `data/` исключён
из git, версионный контроль здесь не запасной путь.

**Ошибка была в приборе, а не в данных**, и это та же порода, что весь день:
проба перебирала не то множество, и её зелёный цвет означал меньше, чем
выглядел. Исправлено: перебор идёт от списка хранилищ, непокрытые называются
первыми, охват печатается числом. Ломка обезвреживает подсчёт непокрытых и
краснит.

**Политика копий — решение оператора 2026-08-25, на прямой вопрос с тремя
вариантами: снимок раз в сутки, хранить семь.** Отвергнуты «только критичные
четыре» (память и причинные записи остались бы без восстановления) и «ничего не
строить».

Построено: `scripts/snapshot_state.py` и вызов из тика. Идемпотентность живёт в
самом снимке, а не в расписании — тик зовёт каждый раз, работа делается раз в
сутки, потому что расписание было бы ещё одним местом для ошибки. Снимок никогда
не роняет тик: страховка, способная остановить работу, хуже её отсутствия.
Живое состояние не трогается — прополка удаляет только собственные старые
поколения.

**И прибор тут же чуть не соврал во второй раз.** Первый снимок лёг, а учение
по-прежнему печатало «19 без копий»: оно знало только `.bak` и о снимках не
подозревало. Ложь прибора и была находкой H-51, поэтому `_backups_for` научен
обоим источникам сразу. После этого охват читается честно: **24 из 24**.
