...
> **FREEZE LIFTED 2026-09-24 — ЗАМОРОЗКА СНЯТА.** Слово оператора, дословно:
> «разморозку сделай полноценно». С этого дня запрет строить новые органы не
> действует, и датчик `tests/test_the_freeze_gates_new_organs.py` больше не
> требует записывать сюда каждый новый модуль. Документ остаётся как история
> решения: два инварианта (кто решает, кто живёт) — принципы архитектуры, а не
> запрет. Условие «не запускать автономный режим как рабочий» снято тем же
> словом: кампания работает с 22.09 по прямому слову оператора.
>
> Почему это всплыло только сейчас. Датчик раскрытия молча пропускался с 19.09:
> история репозитория была обрезана коммитом «Baseline before the 24h
> autonomous run», базовой точки заморозки (`1577b85`) в ней не было, и тест
> уходил в skip. 24.09 полная история нашлась на GitHub
> (`andreyshapira1983-sketch/agent`, 1580 коммитов) — и датчик сразу показал 23
> модуля, построенных за 19–24.09 без записи. Они закрыты таблицей в конце
> документа («Built during the freeze, recorded at the lift»).

**Was — from 2026-08-20 until the lift on 2026-09-24 — the blocking
architectural issue of the project. Kept below as the record of that decision.**
...
`campaign_verdict.py` (2026-09-18, живой прогон владельца: кампания везла
`success_check` в каждой из 232 строк реестра и ни разу по нему не судила —
судья `success_check.py` был подключён к ОДИНОЧНОЙ задаче и не подключён к
кампании. Поэтому слова «достигнуто» в системе не существовало, и страж
повторов был вынужден запирать тему по факту «был цикл с работой».
НОВЫХ ПОЛНОМОЧИЙ НЕТ: орган читает мир и пишет один свой журнал
`data/campaign_verdicts.jsonl`; на вердикт никто пока не опирается, страж
повторов не тронут. Четвёртое слово `preexisting` заведено ЗАМЕРОМ ДО ПРАВКИ,
а не из вкуса: из 31 живого критерия 27 дали `unverifiable`, а 4 дали
`verified`, и все четыре ложны — критерий называл `core/smart_memory.py` как
ПРЕДМЕТ будущей заявки, судья находил этот файл в мире и объявлял цель
достигнутой при нуле заявок. Подключение судьи «как есть» записало бы в вечный
журнал четыре ложных «достигнуто». Разделитель наблюдаем и модели не требует:
след старше начала прогона об этом прогоне не свидетельствует).
...
## Built during the freeze, recorded at the lift (2026-09-24)

Построены между 19.09 и 24.09, когда датчик раскрытия молча пропускался
(обрезанная история), и не были записаны выше. Записаны при снятии заморозки,
чтобы журнал периода был полным. Строка — первая строка описания самого модуля.

| модуль | создан | что это (из описания модуля) |
|---|---|---|
| `core/cache_freshness.py` | 2026-09-19 | Можно ли отдать прошлый результат шага вместо нового вызова. |
| `core/defect_intake.py` | 2026-09-22 | Свой сбой становится дефектом реестра, а не только наблюдением. |
| `core/draft_refutation.py` | 2026-09-22 | Черновик, который противоречит своим же уликам, переписывается один раз. |
| `core/drive_goal.py` | 2026-09-19 | Задача от драйва: что сейчас важно — решает внутреннее состояние, что делать — модель. |
| `core/drives.py` | 2026-09-19 | Внутренние драйвы — физиология вокруг модели, считаемая из журналов. |
| `core/learned_conclusion.py` | 2026-09-19 | Что ход ВЫЯСНИЛ — в долговременную память, а не что он прочитал по пути. |
| `core/memory_consolidation.py` | 2026-09-23 | Сверка нового вывода с памятью ПЕРЕД записью — фаза обновления Mem0. |
| `core/memory_embeddings.py` | 2026-09-23 | Поиск по смыслу для долговременной памяти: multilingual-e5-large-instruct. |
| `core/note_contract.py` | 2026-09-22 | Договор конспекта: что обязано быть в следе учебной задачи, решает код. |
| `core/observation_round.py` | 2026-09-19 | Прочитанное — не конец хода: после успешного пакета шагов решает планировщик. |
| `core/patch_route.py` | 2026-09-22 | Самопочинка без человека в петле: дефект → правка → patch_check → свой код. |
| `core/read_sources_registry.py` | 2026-09-23 | Прочитанное в работе становится источником, а не только конспектом. |
| `core/relevance_judge.py` | 2026-09-22 | Судья относимости: отвечает ли ответ на заданный вопрос — по утверждениям. |
| `core/request_checklist.py` | 2026-09-23 | Чек-лист поручения: требования человека как вопросы «да/нет». |
| `core/self_defect_reminder.py` | 2026-09-21 | Свои открытые записи о промахах — перед глазами в момент действия. |
| `core/split_proof.py` | 2026-09-21 | Доказательство того, что модуль надо переделать, — а не его толщина. |
| `core/splitter_refusals.py` | 2026-09-21 | Журнал отказов раскольщика — пауза для файла, который нельзя разрезать. |
| `core/stuck_route.py` | 2026-09-22 | Уткнулся — спроси: сначала интернет, потом партнёра. |
| `core/write_at_execution.py` | 2026-09-22 | Текст записи собирается в момент исполнения, а не при планировании. |
| `tools/convert_file.py` | 2026-09-24 | `convert_file` — программы для файлов клиента, запущенные безопасно. |
| `tools/find_in_files.py` | 2026-09-19 | Find in Files — поиск по рабочей папке: файлы по имени и строки по тексту. |
| `tools/json_view.py` | 2026-09-19 | Компактный вид JSON-ответа: факты не тонут в длинных полях. |
| `tools/patch_check.py` | 2026-09-22 | `patch_check` — проверить свою правку в отдельной копии, не трогая живой код. |
