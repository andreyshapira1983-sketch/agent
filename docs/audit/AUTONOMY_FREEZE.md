...
`standing_grant.py` (2026-09-18, живой прогон оператора: 26 циклов подряд
`blocked` с одной и той же причиной `approval required`. В ящике 13 заявок:
двенадцать раз автомат просил права на эффекты и ни разу не получил ответа, а
заявок `standing_grant` не было НИ ОДНОЙ за всю историю ящика. Механизм
безнадзорной работы рантайм ЧИТАЛ (`active_standing_grant`), а завести его можно
было только командой в интерактивном `main.py`, куда планировщик не печатает:
читатель без достижимого писателя, зеркало MIR-138.
НОВЫХ ПОЛНОМОЧИЙ У АГЕНТА НЕ ПОЯВЛЯЕТСЯ. Право, которое открывается,
существовало и читалось тем же кодом до этой правки; нова только дверь для
ЧЕЛОВЕКА — флаг `--standing-grant` в `agent_tick.py`. Писатель один на обе двери,
чтобы вторая форма записи не разошлась с читателем (класс H-26).
УСТУПКА НАЗВАНА ВСЛУХ: раньше оператор делал два отдельных жеста
(`:standing-grant`, затем `:approval-approve`), теперь одной командой проходит оба.
Правило B (MIR-117) исполняется не церемонией, а по существу, как в
`rule_approved_apply.py`: просьба и разрешение остаются ДВУМЯ записями, границы
