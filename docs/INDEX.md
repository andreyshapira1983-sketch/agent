...
| **Какие ошибки уже стоили нам денег и времени, и как найти их у себя** | [MISTAKE_NOTEBOOK.md](MISTAKE_NOTEBOOK.md) | пойманные на живых прогонах ошибки: симптом, цена в числах, проверка у себя, что делать | статус дефектов (владелец — `MASTER_ISSUE_REGISTRY.md`); правила поведения (→ `AGENT_DOCTRINE.md`) |
