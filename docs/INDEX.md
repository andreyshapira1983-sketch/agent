...
| **The status of any known defect** | [audit/MASTER_ISSUE_REGISTRY.md](audit/MASTER_ISSUE_REGISTRY.md) | **every issue status — the single owner. 50 issues; its tally is the authoritative count** | design of the fix (→ contract row) |
...
| The **target** memory design | [audit/MEMORY_LIFECYCLE_CONTRACT.md](audit/MEMORY_LIFECYCLE_CONTRACT.md) | the canonical M1 design — **v2-draft, not approved, no code implements it** | what the code does today |
...
- **`docs/audit/` is untracked in git** (5 files, ~2000 lines: this revision's
  registry, progress tracker, inventory, memory map, contract). It is real work
  living outside version control.
- The canonical target-architecture text (`архитектура автономного Агента.txt`,
  repo root) is source-of-truth entry #2 per the README and has not been read in
  full by the current revision.
