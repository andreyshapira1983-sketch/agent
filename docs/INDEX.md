...
| `audit/UNREAD_MAP_2026-09-04.md` | which production files any audit has read, changed or never opened, by static reachability from the unattended and attended entries — the queue for review by blast radius, with the «reachable ≠ observed» caveat | dated snapshot; three states (observed live / reachable not observed / unknown) come with the coverage shift | whoever decides what to audit next |
| `audit/SILENT_FAILURE_REVIEW_2026-09-04.md` | six reviewers over 162 unread files: ~180 candidates, 41 CONFIRMED by execution on the top subset (0 rejected there), four root families, the ratified repair order and the lane containment protocol | dated snapshot; candidates are not facts until a witness | whoever repairs by family |
- [ACTUATION_TEST_2026-09-05](audit/ACTUATION_TEST_2026-09-05.md) — pre-registered actuation test of the agent's routing policy: criteria, baseline (verifier is a dead role), raw policy record, ledger rows, verdict per criterion; transcripts alongside.
- [WORK_ORDER_FLIGHTS_2026-09-05](audit/WORK_ORDER_FLIGHTS_2026-09-05.md) — the first real work order given to the agent (flights TLV→BER), pre-registered rubric and prohibitions, five baskets of dissection, raw result.
