...
class ClaimChunk:
    ...
    verdict: str        # "verified" | "unverified" | "cited_but_unmatched" | "self_declared" | "topic_supported_but_claim_unverified" | "subagent_asserted" | "receipt_missing"
...
class VerificationReport:
    ...
    # Tool receipts slice 1c: evidence cites a slice-1a tool but the
    # ledger has no matching row for this trace_id + operation.
...
def _merge_citation_only_chunks(chunks: list[str]) -> list[str]:
    """Attach standalone citation lines to the preceding claim chunk.

    Some synthesizer models put the required citation on the next line:

        Claim text.
        [file:doc.md]

    The verifier used to split those into an uncited claim plus a verified
    citation-only chunk, producing a false 50/50 verified/unverified pattern.
    This normalizes that layout before verdicting while leaving structural
    headers and metadata sections alone.
    """
    merged: list[str] = []
    for chunk in chunks:
        if (
            _is_citation_only_chunk(chunk)
            and merged
            and not is_structural_chunk(merged[-1])
        ):
...
def verify(
    *,
    ...
    #
    # Receipt slice 1c: demote `verified` chunks whose slice-1a tool
    # evidence lacks a matching ledger row for this trace.
    subagent_asserted = 0
    receipt_missing = 0
    if examined_chunks:
            ...
            if all(_is_derivative_subagent_evidence(e) for e in evs):
                ...
                for i, line in enumerate(annotated_chunks):
                    if line.startswith(ch.text[:30]) and (
                        ...
                        break
                continue
            if matched_evidence_lacks_receipt(
                evs,
                ...
                for i, line in enumerate(annotated_chunks):
                    if (
                        claim_prefix
                        and claim_prefix in line
                        and "[no-receipt]" not in line
                    ):
                        demoted = line.replace("[verified:", "[unverified:")
                        annotated_chunks[i] = demoted.rstrip() + " [no-receipt]"
                        break
                continue
            rebuilt_chunks.append(ch)
