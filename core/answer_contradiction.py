...
#: Что именно записано: адрес файла или его имя. Утверждение о записи обязано
#: называть свой предмет.
#:
#: Замер 2026-09-23, живой прогон: прибор сработал на фразе «Правило записано,
#: но не встало между мной и утверждением» — а правило агент записал ходом
#: РАНЬШЕ, и запись существует (mem_11ce240d85cb768474d8b179180985a1). То есть
#: он докладывал о прошлом факте, а прибор прочёл это как заявление о записи
#: СЕЙЧАС. Ложное срабатывание, и именно его агент сам и предположил, когда
#: просил у оператора разрешение менять детектор.
#:
#: Обратный случай 2026-09-21 пройти НЕ должен: «записано …_v3.md, по измерению
#: 9412 байт» при нуле исполненных инструментов — ложь, и она НАЗЫВАЕТ файл.
#:
#: Отсюда правило, а не список слов: страдательная форма без названного
#: предмета («правило записано», «решение сохранено») — утверждение о
#: СОСТОЯНИИ, а не доклад о действии в этом ходе. Своё действие агент называет
#: от первого лица («записал», «сохранил»), и такая форма остаётся уликой
#: независимо от предмета.
_NAMES_WRITTEN_THING_RE = re.compile(
    r"(?i)[\w./-]+\.(?:md|txt|py|jsonl|json|csv|xlsx|docx|pdf|log|ya?ml|toml)\b"
    r"|\b(?:файл|file)\w*"
)
#: Доклад о СВОЁМ действии: первое лицо. «Записал» — это я сейчас, «записано»
#: — это состояние мира, возможно давнее.
...
def action_report_mismatch(answer: str | None, executed_tools: list[str]) -> str | None:
    ...
    # Вторая ветка того же вопроса. Правка 2026-09-23 сперва тронула только
    # первую (ход без инструментов ВООБЩЕ), а живое ложное срабатывание шло
    # именно отсюда — ход, где инструменты были, но записи не было. Поймано
    # своим же тестом; тот же промах, что утром с учётом кэша: путей два,
    # правка легла в один.
