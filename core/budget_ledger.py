...
class BudgetLedger:
    ...
    def from_env(
        ...
        *,
        ...
        return cls(path=path, windows=_windows_from_env(), logger=logger)
...
def _windows_from_env() -> tuple[BudgetWindow, ...]:
    hour_limits = _limits_from_env("AGENT_BUDGET_HOUR")
    day_limits = _limits_from_env("AGENT_BUDGET_DAY")
    return (
...
def _limits_from_env(prefix: str) -> dict[BudgetLedgerCounter, int]:
    return {
        "llm_calls": _env_int(f"{prefix}_LLM_CALLS"),
        "model_tokens": _env_int(f"{prefix}_MODEL_TOKENS"),
        "model_cost_units": _env_int(f"{prefix}_MODEL_COST_UNITS"),
        "web_fetches": _env_int(f"{prefix}_WEB_FETCHES"),
        "cycles": _env_int(f"{prefix}_CYCLES"),
        "team_model_calls": _env_int(f"{prefix}_TEAM_MODEL_CALLS"),
        "team_cost_units": _env_int(f"{prefix}_TEAM_COST_UNITS"),
    }
