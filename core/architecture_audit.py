...
def _build_checks(workspace: Path) -> list[ArchitectureLayerCheck]:
    return [
        ...
        _check(
            workspace,
            id="smart_experience_memory",
            title="Episodic / Procedural / Consolidation Memory",
            evidence=("core/smart_memory.py", "core/loop.py", "main.py"),
            tests=("tests/test_smart_memory.py",),
            summary="Completed cycles become episodes, successful tool workflows become procedures, and consolidation reports link them.",
            next_step="Use consolidated procedures inside long work-session reports and subagent scopes.",
        ),
        _check(
            workspace,
        ...
        _check(
            workspace,
            id="capability_request_layer",
            title="Autonomous Capability Request Layer",
            evidence=("core/capability_request.py", "main.py"),
            tests=("tests/test_capability_request.py", "tests/test_cli.py"),
            summary="Natural-language goals can produce bounded connector/capability requests before any access is activated.",
            next_step="Add approval-to-capability activation after request boundaries are stable.",
        ),
        _check(
            workspace,
        ...
        _check(
            ...
            summary="Release manifest excludes local secrets and CI enforces hash-locked dependencies plus SBOM sync.",
            next_step="Add vulnerability/dependency review scanning after the hash lock remains stable.",
        ),
        _check(
            workspace,
