...
def _others_can_write(path: Path) -> bool:
    """В метку может писать кто угодно — её могли подменить.

    По мотивам StrictModes в OpenSSH: файл, открытый на запись всем, sshd не
    читает. 24.09 метка на сервере лежала rw-rw-rw-; закрывал её только каталог
    /root. Запись ГРУППОЙ не отклоняется: на сервере маска 0002, и группа там —
    root; отклонять её значило бы отклонять каждую свежую метку (проверено
    полным прогоном до отправки). На Windows битов нет — проверки нет.
    """
    if os.name != "posix":
        return False
    try:
