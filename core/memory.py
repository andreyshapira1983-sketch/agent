...
class WorkingMemory:
    ...
    def conversation_context(self, max_turns: int = 5) -> str:
        ...
        Guarantees the result is no longer than `self.max_context_chars`.
        """
        if not self.turns:
            return ""
        ...
        if len(text) > self.max_context_chars:
            # Drop the oldest chunks until we fit. Worst case we keep nothing
            # and return a short tombstone.
            while len(text) > self.max_context_chars and len(chunks) > 1:
                chunks.pop(0)
                text = "\n\n".join(chunks)
            if len(text) > self.max_context_chars:
                text = text[: self.max_context_chars - 1].rstrip() + "…"
        return text
    ...
    def cache_store(
        ...
        self.artifacts[self.cache_key(tool_name, arguments)] = {
            "tool": tool_name,
            "arguments": arguments,
            "output": output,
            "label": label,
