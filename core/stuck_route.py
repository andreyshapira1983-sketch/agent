"""Уткнулся — собери улики и реши; человека — только когда шаг не твой.

(Название модуля старое: до 2026-09-26 здесь было «спроси: сначала интернет,
потом партнёра», и вопрос человеку считался успехом — см. stuck_goal.)
...
def stuck_goal(root: Path | str) -> Any:
    """Цель «уткнулся»: собрать улики и ЗАПИСАТЬ решение; человек — только не своё.

    До 2026-09-26 успехом цели был вопрос человеку («в chat_outbox появилась
    запись»), а правило «когда звать» жило здесь отдельно от правила голоса.
    Ночь 26.09: ответ лежал в его уликах, он спросил «да/нет» и ходил по кругу.
    Правило теперь одно (core/own_decisions.py), успех — ЭТО решение по номеру:
    файл журнала существует всегда, и критерий «файл есть» засчитывал любой исход.
    """
    import hashlib
