...
`core/causal_lesson.py` описывает подъём OBSERVED → EXPLAINED → ATTRIBUTED →
GENERALIZED → LESSON. Собиралась только нижняя ступень, и та писалась в журнал:
`_record_causal_observation` прямо говорит «в хранилище ничего не кладётся».
Значит подниматься было не с чего — MIR-096.

Здесь лежит ровно нижняя ступень и ничего выше. Запись означает «стоит
расследовать», а не «доказано»: тег `lesson` зарабатывается через `state_of`, и
никакая запись сюда его не выдаёт.

Повторы схлопываются по отпечатку. Четырнадцать одинаковых `dirty_tree_wait`
ничему не научили именно потому, что были четырнадцатью строками (MIR-090):
повторение — это сигнал, что дефект есть класс, а не случайность, и он должен
читаться числом, а не пересчётом строк.

Зачем и что дальше: docs/CODE_NOTES.md, «The first rung is surviving the turn».
"""
...
def observation_fingerprint(observation: Observation) -> str:
    ...
    Сигналы детекторов, а не текст: формулировка `observed_mismatch` содержит
    имена прогонов и меняется от хода к ходу, поэтому сверка по ней плодила бы
    новую запись на каждое повторение — ровно та беда, от которой эта запись
    заводится.
    """
