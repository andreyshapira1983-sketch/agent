...
#: Строка выдачи find_in_files: «path:line: text».
_HIT_LINE = re.compile(r"^(\S+?):\d+: ")


def _observed_blob(artifacts: dict[str, Any]) -> str:
    """Что ход ДЕЙСТВИТЕЛЬНО наблюдал: метки шагов плюс файлы, прошедшие через них.

    Замер 2026-09-19 (опыт с библиотекой книг): «сколько раз слово entropy
    встречается в Tong_StatisticalPhysics.txt» — лаборатория получила этот
    файл и верно насчитала 131, но метка её шага — начало кода
    («python_probe:import re»), и долг наблюдения за файлом числился
    `silently_missing`. Все три вопроса на подсчёт теряли допуск в опыт.
    Наблюдением считается файл, который лаборатория получила (`inputs`), и
    файл, в котором поиск нашёл строки; имя в тексте запроса — не наблюдение.
    """
    # Метка поиска несёт текст ЗАПРОСА — это не наблюдение; для него в счёт
    # идут только пути найденных строк.
    parts = [str(k) for k, meta in artifacts.items() if (meta or {}).get("tool") != "find_in_files"]
    for meta in artifacts.values():
        meta = meta or {}
        out = meta.get("output")
        if meta.get("tool") == "python_probe" and isinstance(out, dict):
            parts += [str(p) for p in out.get("inputs") or ()]
        elif meta.get("tool") == "find_in_files" and isinstance(out, str):
            lines = out.splitlines()[1:]  # первая строка — сводка, в ней текст запроса
            parts += [m.group(1) if (m := _HIT_LINE.match(ln)) else ln.strip() for ln in lines]
    return " ".join(parts)
...
def evaluate_completion_obligations(
    *,
    ...
    artifact_blob = _observed_blob(artifacts)
    ...
    if contract is not None:
        ...
        unavailable = ()
        unmet = set(unmet_obligations(contract, artifacts=artifacts))
        obligations.extend(
            Obligation(
                source="acceptance_criteria",
                kind="deliverable_produced",
                status=(
                    "satisfied" if duty not in unmet
                    else "failed_but_reported"
                    if failure_codes
                    and _discloses(answer, duty.target or "tests", *failure_codes)
                    else "silently_missing"
                ),
                detail=(
                    f"{duty.deliverable}:{duty.target or 'tests'}"
                    f" — {duty.verification}"
                ),
            )
