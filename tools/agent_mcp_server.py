#!/usr/bin/env python3
"""Read-only MCP view of this agent, so an assistant session can see it.
...
Registered for Claude Code sessions in this repository by `.mcp.json`.
"""
from __future__ import annotations

import json
import subprocess  # nosec B404 — one fixed argv, no shell, no external input
import sys
...
def agent_status() -> str:
    ...
    Exactly what `python agent_tick.py --status` prints — the agent's own
    summary of itself, not a second implementation that could drift from it.
    """
    try:
        proc = subprocess.run(  # noqa: S603  # nosec B603
            [sys.executable, str(REPO / "agent_tick.py"), "--status"],
    ...
    if proc.returncode != 0:
        return (f"status exited {proc.returncode}\n"
