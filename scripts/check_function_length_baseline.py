...
WATCH: dict[str, int] = {
    ...
    "core/best_next_action.py:select_best_next_action": 159,  # +3 (2026-08-30): :self-issue-retire — дверь человеческого вердикта; та же плоская таблица  # +4 (2026-08-28): ветка :receipts — 95-я команда, читатель улики MIR-138; диспетчер растёт с каждой командой по построению  # прежний потолок 351  # +2 (2026-08-26): :standing-grant. Забанковано, а не срезано, и это единственный банк за неделю: диспетчер — плоская таблица, где КАЖДАЯ команда стоит две-три строки, и храповик здесь ловит не разрастание обязанностей, а прибавление строки в таблицу. Настоящее лекарство — диспетчеризация по словарю, а не дробление таблицы пополам.
