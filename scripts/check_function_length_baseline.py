...
WATCH: dict[str, int] = {
    ...
    "core/verifier_core.py:verify": 320,  # +10 (2026-08-12): вердикт `refuted` — доказанная ложь перестала быть разновидностью «не подтверждено»; ветка вердикта обязана жить в цепочке вердиктов, её владелец — эта функция (docs/CODE_NOTES.md «REFUTED is a polarity»).  # +6 (2026-08-10): пятый гейт содержания — утверждение об отсутствии, опровергнутое собственной уликой; тело в verifier_utils.  # +4 (2026-08-10): четвёртый гейт содержания (MIR-060, класс «литерал есть в утверждении и отсутствует в улике»); тело вынесено в verifier_utils, здесь только вопрос и ответ.
