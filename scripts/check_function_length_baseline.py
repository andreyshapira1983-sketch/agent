...
WATCH: dict[str, int] = {
    ...
    "core/loop.py:AgentLoop._run_inner": 428,  # +1 (2026-09-04 exam, turn 2: the spend/roster block now reaches the synthesizer through SynthesisState).  # замер 417 (2213 до раскола) + запас 10
    "core/step_sanitizer.py:sanitize_step": 834,  # +22 (2026-09-04 22:20, his routing door «model_route»: role/provider/reason required, model unless release).  #+19 (2026-09-04 night, the read door «memory_recall»: one string argument, same shape as memory_bank).  #+17 (2026-09-04, the eye «model_roster»): one more argument-less branch, same shape as current_time; the registered-tool guard demands it.  # +42 (2026-09-01, Fable строит за агента): ветка semantic_scholar_search — лечение МЁРТВОЙ способности: инструмент стоял в поясе и в промпте планировщика, а шаг с ним молча выбрасывался. Найден новым сторожем tests/test_a_registered_tool_is_not_silently_dead.py. Лекарство остаётся прежним и всё нужнее: диспетчеризация по словарю (названо агентом 2026-09-01).  # +37 (2026-09-01): ветка journal_append — легитимный обработчик, не раздувание; настоящее лекарство названо — диспетчеризация по словарю (решение и подпись — агента, append_ratchets.md).  # +29 (2026-08-31): added memory_bank dispatch block; one tool branch is not a reason to cut (решение и подпись — агента, sanitizer_ratchet.md).
