...
WATCH: dict[str, int] = {
    ...
    "core/best_next_action.py:select_best_next_action": 167,  # +8 (2026-09-03, Д3 по слову оператора): четвёртый генератор по цели (цель называет собственный дефект — руки этого дефекта) и отвод привычки реестра под целью, назвавшей другую запись или конкретную вещь; логика вынесена в _habit_shadowed_by_goal, в таблице осталась строка допуска и четыре строки отвода. Замер: три прогона подряд 03.09 отдавали свою цель чужой записи.  # +3 (2026-08-30): :self-issue-retire — дверь человеческого вердикта; та же плоская таблица  # +4 (2026-08-28): ветка :receipts — 95-я команда, читатель улики MIR-138; диспетчер растёт с каждой командой по построению  # прежний потолок 351  # +2 (2026-08-26): :standing-grant. Забанковано, а не срезано, и это единственный банк за неделю: диспетчер — плоская таблица, где КАЖДАЯ команда стоит две-три строки, и храповик здесь ловит не разрастание обязанностей, а прибавление строки в таблицу. Настоящее лекарство — диспетчеризация по словарю, а не дробление таблицы пополам.
    ...
    "app/bootstrap.py:build_agent": 258,  # +2 (2026-09-01): регистрация journal_append — обязательная точка подключения, не дублирование (решение агента, append_ratchets.md).  # +2 (2026-08-31): регистрация memory_bank — банка памяти агента (решение и подпись — агента, ratchet_entry.md).  # +2 (2026-08-17): регистрация lesson_provenance — прибора причинного происхождения уроков.  # +2 (2026-08-16): регистрация python_probe — лаборатории замера собственной среды.  # 246 -> 250 (2026-08-15): импорт, создание и передача causal_store — без него наблюдение не переживало ход (MIR-096).
