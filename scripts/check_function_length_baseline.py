...
WATCH: dict[str, int] = {
    ...
    "core/verification_summary.py:build_verification_summary": 156,  # 156 (2026-09-23): впервые пересекла порог 150 — в неё въехал ОДИН счёт подтверждённого на весь дом: опора на разговор теперь в числителе отчёта, как она уже была в числителе ворот обрезки и compute_evidence_support. Замер: агент выбрал заказ верно, а внизу стояло «подтверждено 0 из 8, уверенность нулевая», потому что все опоры были из разговора — отчёт наказывал их дважды. Дробить не за что: функция собирает ПЯТЬ пунктов одного объяснения, и разнести их значит развести по файлам то, что читается вместе; правило оператора — дробим за лишние обязанности или нечитаемость, никогда за длину.
    "core/planner.py:LLMPlanner.plan": 153,  # 153 (2026-09-23): впервые пересекла порог 150 — в неё въехали две строки, убирающие из шапки описания скрытых инструментов. Дробить не за что: обязанность одна, а правило оператора — дробим за лишние обязанности или нечитаемость, никогда за длину.
    "core/loop.py:AgentLoop._run_inner": 319,  # −113 (2026-09-26): из функции убрана писанина, код тот же.  # +1 (2026-09-21): per-run reset of _self_defects_block — the agent's own open mistakes reach the synthesizer; a stale block from the previous turn must not ride into the next (tests/test_cross_mixin_fields_are_guaranteed.py sees only a plain assignment).  # +1 (2026-09-19 web exam): the failure history reaches the response draft, so a turn stuck on its own draft's citations does not ask the human «what to build».  # +2 (2026-09-04 exam, turn 3: the sensor blocks folded into the evidence chain).  # +1 (2026-09-04 exam, turn 2: the spend/roster block now reaches the synthesizer through SynthesisState).  # замер 417 (2213 до раскола) + запас 10
    "core/step_sanitizer.py:sanitize_step": 771,  # +6 (2026-09-25, правило оператора «помощник — по обоснованию»): вызов _subagent_reason и два поля why/expect в аргументах; проверка вынесена в _subagent_reason.  # +3 (2026-09-24, слово оператора «делай безопасный инструмент для программ»): развилка convert_file -> `_sanitize_convert_file`, тело в tools/convert_file.py; здесь только диспетчер.  # +2 (2026-09-23): файл правки готовым текстом не пишется — планировщик чужого файла не видел. Тело замены вынесено в `_patch_task_from`, здесь остались две строки вызова. Замер: 28 записей правок из 28 пришли с сочинённым текстом, пять правок умерли с «the patch did not apply».  # +5 (2026-09-22, корень «сочиняет запись до чтения»): file_write принимает write_instruction вместо готового текста — текст собирается при исполнении (core/write_at_execution.py).  # +3 (2026-09-22): развилка patch_check -> `_sanitize_patch_check` (тело вынесено, здесь только диспетчер).  # -41 (2026-09-20, вечер): ветка web_fetch ушла в `_sanitize_web_fetch`. Лекарство названо в этой же строке давно — диспетчеризация вместо ветки на инструмент, — и каждая правка до сих пор вместо него поднимала планку. Планка опущена, чтобы выигрыш нельзя было проесть обратно.  # +2 (2026-09-20): условие «путь существует в рабочей папке» у запрета на не-ASCII: запрет писался против ВЫДУМАННЫХ планировщиком имён, а резал 1556 собственных математических черновиков агента — 20 снятых чтений за семь суток. Проверка существования вынесена в `_exists_in_workspace`, здесь остался вызов.  # +3 (2026-09-19, опыт с библиотекой книг): ветка find_in_files — диспетчер к `_sanitize_find_in_files`, тело вынесено; инструмент без санитайзера выбрасывается планом.  # -46 (2026-09-05 evening, the read_logs branch moved out to `_sanitize_read_logs`, where last_n is clamped instead of dropped; the file_read window lives in `_line_range_arguments` — the first two branches to leave the ladder).  # +22 (2026-09-04 22:20, his routing door «model_route»: role/provider/reason required, model unless release).  #+19 (2026-09-04 night, the read door «memory_recall»: one string argument, same shape as memory_bank).  #+17 (2026-09-04, the eye «model_roster»): one more argument-less branch, same shape as current_time; the registered-tool guard demands it.  # +42 (2026-09-01, Fable строит за агента): ветка semantic_scholar_search — лечение МЁРТВОЙ способности: инструмент стоял в поясе и в промпте планировщика, а шаг с ним молча выбрасывался. Найден новым сторожем tests/test_a_registered_tool_is_not_silently_dead.py. Лекарство остаётся прежним и всё нужнее: диспетчеризация по словарю (названо агентом 2026-09-01).  # +37 (2026-09-01): ветка journal_append — легитимный обработчик, не раздувание; настоящее лекарство названо — диспетчеризация по словарю (решение и подпись — агента, append_ratchets.md).  # +29 (2026-08-31): added memory_bank dispatch block; one tool branch is not a reason to cut (решение и подпись — агента, sanitizer_ratchet.md).
    "core/loop_step_execution.py:AgentLoopStepExecution._execute_step": 569,  # +1 (2026-09-23): воротам передаются файлы висящих заявок — одна строка аргумента, ради соразмерности запрета (слово оператора).
    ...
    "core/loop_memory_read.py:AgentLoopMemoryRead._retrieve_experience_memory": 159,  # −61 (2026-09-26): из функции убрана писанина, код тот же.  # +2 (2026-08-29, MIR-184): проводка сшивки семьи — вызов и дописка приложения; сама логика вынесена в _family_product_warnings/_family_appendix
    ...
    "core/loop_attempt.py:AgentLoopAttempt._run_attempt_loop": 443,  # +1 (2026-09-24): у короткого пути второй распознаватель — классификатор болтовни (core/social_turn.py), слово оператора «шутка не запускает конвейер».  # +3 (2026-09-20): журнал называет, чем судили согласованность (довод шага или запасной словарь) и сколько шагов пришли с доводом — иначе два способа неотличимы в записях.  # +1 (2026-09-20): датчик согласованности судит по доводу ШАГА, когда план его несёт; словарь по прозе остался запасным путём.
