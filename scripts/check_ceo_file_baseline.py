...
WATCH: dict[str, int] = {
    ...
    "core/autonomous_runtime.py": 1527,  # +27 (2026-08-15): затвор эффектов ЧИТАЕТ ответ человека. До этого `effects_approved` ставился ровно в одном месте (`:approval-run`), кампания туда не ходит — она заводила заявку, блокировалась, и следующий прогон заводил новую с тем же dedup_key. Одобрение было письмом в никуда: живой круг — одобрено ain_369d8fdb, прогон встал на ain_bea806cc. Разрешение осталось одноразовым (executed), право §9 у человека (docs/CODE_NOTES.md «The approval nobody read»).    # measured 1495; aspiration 1150
