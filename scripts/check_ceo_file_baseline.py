...
WATCH: dict[str, int] = {
    ...
    "core/smart_memory.py": 1922,  # +3 (2026-08-10): вердикт понижают ДВА сигнала — непредставленный контракт и неадресованные названные единицы.          # measured 1861 after the causal-credit split + outcome extraction +7 (2026-08-10): один предикат `_answer_disqualified` на ДВА рубежа — допуск эпизода и кредит процедуры. Порознь они уже разошлись, и самоопровергнувшийся прогон поднял счётчик активной процедуры. +12 (2026-08-10): вердикт завершения перестал удостоверять то, чего не проверял — `user_contract_partial` понижает заявленное `achieved`, как и `obligation_unmet`, и той же односторонней властью.
