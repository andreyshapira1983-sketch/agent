...
WATCH: dict[str, int] = {
    ...
    "agent_tick.py": 1765,                 # +21 (2026-08-24, H-38): _is_failure_event — пульс пишется и на ветке ОТКАЗА, поэтому свежесть отметки говорила «жив», а правда лежала в скобках. Сквозной замер: три подряд упавших тика на испорченном config/budget_limits.json печатали «Daemon: alive — last tick 0.0 min ago (event=tick_error)». Решение о вечном повторе не менялось (MIR-135 открыт отдельно) — изменено слово, которым строка ведёт, на существующей поверхности.  # +48 (2026-08-24, H-33): _require_budget_config — весь денежный потолок агента держится на ОДНОМ файле config/budget_limits.json (лимитов в окружении нет ни одного), и при его отсутствии окна остаются с нулевыми пределами: замер показал 500 разрешённых вызовов подряд, молча. Тот же файл с ПОРЧЕНЫМ содержимым при этом бросает — то есть из двух ошибок настройки тихо проходила ровно та, что СНИМАЕТ ограничение. Место проверки найдено с третьей попытки: слой библиотеки покраснил 168 тестов, run_tick — 17, и обе правы; настоящая граница — вход процесса, куда безнадзорный тик приходит и куда тесты не ходят.  # +40 (2026-08-24, H-29): _quarantine_status_lines — порченая строка состояния уезжает в карантин, файл переписывается без неё, и читатель получает список короче, ничем не отличая его от полного. У обычного журнала это потеря памяти; у счётчика-ПОТОЛКА это выданное разрешение: замер показал, что исчерпанный суточный потолок в три вызова после порчи одной строки снова разрешает тратить. Решение (разрешать или отказывать) не менялось — оно про деньги и принадлежит оператору; закрыто молчание, и закрыто на поверхности, которую оператор и так читает, а не новым журналом без читателя (MIR-138).  # +27 (2026-08-24, H-27): _charter_goal_router — построение роутера выбора цели вынесено из main и достроено до связки из app/bootstrap.py. Замер по живым данным: резервов llm_calls 1364 против 1379 строк расхода, разрыв весь в openai (659/644) и весь на 19-20 августа; те же 15 строк отсутствуют среди model_cost_units. Причина — леджер строился здесь своей строкой, без budget_ledger, а резерв и запись стоимости стоят под `budget_ledger is not None`: вызов был виден в учёте и невидим для суточного и недельного ПОТОЛКА. Правка 19 августа чинила ту же строку и закрыла лишь измеренную половину. Рост — цена того, что построение денежного пути теперь именуемо и проверяемо тестом, а не строка внутри ветки main.  # +49 (2026-08-22, ревизия закрытий): _provider_health_line — оператор впервые видит, какого провайдера роутер пропускает. Найдено НЕ новым дефектом, а проверкой закрытия MIR-132 против названных полем провалов этого класса: «выключатель без наблюдаемости не настраивается» — наш был невидим. Первый черновик строки печатал «anthropic ok» провайдеру с 391 отказом по балансу (устаревшие отказы выпадают из окна остывания), поэтому строка показывает последний исход, а не только состояние остывания.  # +31 (2026-08-22, вторая правка дня): _sweep_episodic_duplicates — ровно ОДИН из тринадцати CLI-only органов MIR-131 переведён на безнадзорный путь, и граница проведена по суждению: схлопывание байт-идентичных дублей механично (новейший из одинаковых, потерь нет), а чистка по «ценности» — это суждение о собственной памяти (опасность кресла-резолвера, MIR-128) и остаётся оператору. Счёт класса: 43 одинаковых эпизода за один безнадзорный день.  # +8 (2026-08-22): починка застрявших строк очереди. Строка, брошенная умершим процессом, и строка, припаркованная исчерпанным бюджетом, — два состояния покоя с одним договором (общий замок, только старт), и второе не имело автоматического выхода вовсе: 14 строк, старейшая с 2026-07-30. Оба прохода уехали в _free_stranded_rows, поэтому run_tick НЕ вырос, а сократился 497 → 452 (ветвей 35 → 33, операторов 167 → 162); рост файла — цена выноса, а не длины функции.  # 1540 → 1541 (2026-08-19): докстринг честно называет реальный дефолт провайдера (аудит перед пушем).  # 1528 → 1540 (2026-08-19, вскрытие 19:31): _ensure_env_loaded — плановые тики выбирали цель до загрузки .env, то есть на модели по умолчанию.  # 1525 → 1528: леджер на чартерный путь — скрытый расход Sol стал видимым.  # measured 1522 (2026-08-15: --charter
